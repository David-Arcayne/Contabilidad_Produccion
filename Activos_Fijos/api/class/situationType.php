...
class SituationType
{
    ...
    public function getById($situation_id){
        $valid = new Validations($this->t_name);

        $valid_id = $valid->isNumber($situation_id, "id");
        if($valid_id) {
            echo json_encode([
                "status" => 400,
                "error" => $valid_id
        ...
        try {
            ...
            $consulta->bind_param("s", $situation_id);
            $consulta->execute();
    ...
    public function edit($situation_id){        
        ...
        if($error_arr) {
            ...
            try {
                ...
                $value_columns = array_values($v_form);
                array_push($value_columns, $situation_id);
    ...
    public function delete($situation_id){
        ...
        try {
            ...
            $quer->bind_param("i", $situation_id);
            $quer->execute();
    ...
    public function report($js = null){
        try {
            ...
            if (!$js) {
                ...
                $object->reportSituationType($this->res);
                return;
            }
        } catch (Throwable $th) {
            $this->res = [
