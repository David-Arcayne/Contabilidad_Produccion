...
class Worker
{
    ...
    public function __construct()
    {
        ...
        $this->t_name = "trabajador"; 
        $this->col_id = "idtrabajador"; 
    ...
    public function getAllEnabled(){
        try {
            ...
            $quer_trabajador=$this->db_rh->prepare("SELECT idtrabajador, CONCAT(nombre, ' ', apellido) AS nombreapellido FROM trabajador WHERE cargos_idcargos IN (SELECT idcargos FROM cargos WHERE areas_idareas IN (SELECT idareas FROM areas WHERE sucursal_idsucursal IN ($string_su_id)))");
    ...
    public function getAllEnabledByArea($area_id){
        try {
            $quer_trabajador=$this->db_rh->prepare("SELECT idtrabajador, CONCAT(nombre, ' ', apellido) AS nombreapellido FROM trabajador WHERE cargos_idcargos IN (SELECT idcargos FROM cargos WHERE areas_idareas = ?)");
            $quer_trabajador->bind_param("i", $area_id);
            $quer_trabajador->execute();
            $result_t =  $quer_trabajador->get_result();
            $data = $this->db_rh->all($result_t, MYSQLI_ASSOC);
