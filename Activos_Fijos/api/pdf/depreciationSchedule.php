...
class DepreciationSchedule {
    ...
    function periodoFiscal() {
        $fecha = $this->datosEmpresa->ocierrefiscal;
        if (empty($fecha)) {
            return "2000-12-31";
        }

        $fechaFormateada = DateTime::createFromFormat("d/m/Y", "$fecha/2000");
    ...
    public function StraightLine($data, $previousData = [], $mix = NULL){ 
        ...
        $enterpriseDate = $this->periodoFiscal();
        ...
        foreach ($categories as $keyC => $category) {
                ...
                if($fixedAsset["categorias_id"] ==  $category["id"]){
                    ...
                    if ($dateFA < $dateDepreciation && $month_fis_dep > 0) {
                        ...
                        for ($y = $newYearFA; $y <= $yearDepreciation; $y++) { 
                            ...
                            //     && strtotime(date("Y-m-d", strtotime($dateOldDep))) < strtotime(date("Y-m-t", strtotime($y ."-". ($monthFA - 1) ."-1")))
                            //     && !$notUsefulLife
                            // ) 
                            ...
                            $boolOD = ($dateOldDep ? ( (strtotime($dateOldDep) < strtotime($finishDateFA)) ? TRUE : FALSE) : TRUE);
                            $monthDiffFor = $this->diffMonths(date("Y-m-t", strtotime($y ."-". ($monthFA - 1) ."-1")), $dateDepreciation);
                            // echo '| '.print_r(":: After-Depreciar = ".($yearFA + $newUsefulLife) ." - ". $finishDateFA ." - ". $monthDiffFor ." - ". date("Y-m-t", strtotime($y ."-". ($monthFA - 1) ."-1")) , true).' |';
                            ...
                            )
                            {
                                ...
                                $realM2 = $dateOldDep ? $this->diffMonths($dateOldDep, $finishDateFA) : 0;
                                $realMonthL = ($dateOldDep && $realM2 > 0 && strtotime($initialDateFA) <= strtotime($dateOldDep)) ? $realM2 : $this->diffMonths($initialDateFA, $finishDateFA);
                                // echo '| '.print_r($finishDateFA ." - ". $realMonthL ." - ". $realM2 ." - ". $initialDateFA, true).' |';
                                $realM = $this->diffMonths($dateFA, $finishDateFA);
                                // $realMonthL = ($dateOldDep && $this->diffMonths($dateOldDep, $dateDepreciation) > 0) 
                            ...
                            if ($y == $yearDepreciation) {
                                ...
                                if ($this->diffMonths($fiscalDateY, $dateDepreciation, false) > 0){
                                    ...
                                    //     $testRM = $dateOldDep ? $this->diffMonths($dateOldDep, $dateDepreciation, false) : -1;
                                ...
                                if ($existUsefulLife) {
                                    ...
                                    $testRM = $dateOldDep ? $this->diffMonths($dateOldDep, $dateDepreciation, false) : -1;
                                    $realMonth = $testRM >= 0 && $testRM < 12 ? $testRM : $this->diffMonths($initialDateFA, $dateDepreciation, false); // TODO: posiblemente igual a :: Depreciar (mantener en 'false' ya que en :: Depreciar se resta un mes)
                    ...
                    if (!($this->gtNW || $this->stNW) || $showData === TRUE) {
                        ...
                        <tr class="afr-tr-e">
                            ...
                            <td>'. ( $isRevaluation ? "" : $pdf->FormatoNumber($incomeValue) ) .'</td>
                            <td>'. ( $isRevaluation ? "" : $pdf->FormatoNumber($salvageValue) ) .'</td>
                            <td>'. $pdf->FormatoNumber($revaluation) .'</td>
                            <td>'. $pdf->FormatoNumber($updatedValueOld) .'</td>
                            <td>'. $pdf->FormatoNumber($update) .'</td>
                            <td>'. $pdf->FormatoNumber($updatedValue) .'</td>
                            <td>'. $pdf->FormatoNumber($depreciation) .'</td>
                            <td>'. $pdf->FormatoNumber($accumDeprOld) .'</td>
                            <td>'. $pdf->FormatoNumber($updateAD) .'</td>
                            <td>'. $pdf->FormatoNumber($accumDeprU) .'</td>
                            <td>'. $pdf->FormatoNumber($accumDepr) .'</td>
                            <td>'. ( round(($updatedValue - $accumDepr), 2) == 0 ? 1 : $pdf->FormatoNumber($updatedValue - $accumDepr) ) .'</td>
                        </tr>';
                        $partialPrice += $isRevaluation ? 0.00 : floatval($incomeValue);
                        $partialSalvage += $isRevaluation ? 0.00 : round((float)$salvageValue, 2);
                        $partialRevaluation += round((float)$revaluation, 2);
                        $partialUpdatedValueOld += round((float)$updatedValueOld, 2);
                        $partialUpdate += round((float)$update, 2);
                        $partialUpdatedValue += round((float)$updatedValue, 2);
                        $partialDepreciation += round((float)$depreciation, 2);
                        $partialAccumDeprOld += round((float)$accumDeprOld, 2);
                        $partialUpdateAD += round((float)$updateAD, 2);
                        $partialAccumDeprU += round((float)$accumDeprU, 2);
                        $partialAccumDepr += round((float)$accumDepr, 2);
                        $partialNetWorth += round(($updatedValue - $accumDepr), 2) == 0 ? 1 : round((float)($updatedValue - $accumDepr), 2);
                        
                        $totalPrice += $isRevaluation ? 0.00 : floatval($incomeValue);
                        $totalSalvage += $isRevaluation ? 0.00 : round((float)$salvageValue, 2);
                        $totalRevaluation += round((float)$revaluation, 2);
                        $totalUpdatedValueOld += round((float)$updatedValueOld, 2);
                        $totalUpdate += round((float)$update, 2);
                        $totalUpdatedValue += round((float)$updatedValue, 2);
                        $totalDepreciation += round((float)$depreciation, 2);
                        $totalAccumDeprOld += round((float)$accumDeprOld, 2);
                        $totalUpdateAD += round((float)$updateAD, 2);
                        $totalAccumDeprU += round((float)$accumDeprU, 2);
                        $totalAccumDepr += round((float)$accumDepr, 2);
                        $totalNetWorth += round(($updatedValue - $accumDepr), 2) == 0 ? 1 : round((float)($updatedValue - $accumDepr), 2);
            ...
            if ($cont != 0) {
                ...
                <tr class="afr-tcategory">
                    ...
                    <th ></th>
                    <th ></th>
                    <th >'. $pdf->FormatoNumber($partialPrice) .'</th>
                    <th >'. $pdf->FormatoNumber($partialSalvage) .'</th>
                    <th >'. $pdf->FormatoNumber($partialRevaluation) .'</th>
                    <th >'. $pdf->FormatoNumber($partialUpdatedValueOld) .'</th>
                    <th >'. $pdf->FormatoNumber($partialUpdate) .'</th>
                    <th >'. $pdf->FormatoNumber($partialUpdatedValue) .'</th>
                    <th >'. $pdf->FormatoNumber($partialDepreciation) .'</th>
                    <th >'. $pdf->FormatoNumber($partialAccumDeprOld) .'</th>
                    <th >'. $pdf->FormatoNumber($partialUpdateAD) .'</th>
                    <th >'. $pdf->FormatoNumber($partialAccumDeprU) .'</th>
                    <th >'. $pdf->FormatoNumber($partialAccumDepr) .'</th>
                    <th >'. $pdf->FormatoNumber($partialNetWorth) .'</th>
                </tr>';
        ...
        <tr class="afr-tcategory">
            ...
            <th >'. $pdf->FormatoNumber($totalPrice) .'</th>
            <th >'. $pdf->FormatoNumber($totalSalvage) .'</th>
            <th >'. $pdf->FormatoNumber($totalRevaluation) .'</th>
            <th >'. $pdf->FormatoNumber($totalUpdatedValueOld) .'</th>
            <th >'. $pdf->FormatoNumber($totalUpdate) .'</th>
            <th >'. $pdf->FormatoNumber($totalUpdatedValue) .'</th>
            <th >'. $pdf->FormatoNumber($totalDepreciation) .'</th>
            <th >'. $pdf->FormatoNumber($totalAccumDeprOld) .'</th>
            <th >'. $pdf->FormatoNumber($totalUpdateAD) .'</th>
            <th >'. $pdf->FormatoNumber($totalAccumDeprU) .'</th>
            <th >'. $pdf->FormatoNumber($totalAccumDepr) .'</th>
            <th >'. $pdf->FormatoNumber($totalNetWorth) .'</th>
        </tr>';
        ...
        foreach ($categories as $keyC => $category) {
            ...
            for ($i=$fa_array_position; $i < sizeof($fixedAssets); $i++) { 
                ...
                if($fixedAsset["categorias_id"] ==  $category["id"]){
                    ...
                    // echo '| '.print_r($arrayYearSumDigits, true).' |';
                    ...
                    if ($dateFA < $dateDepreciation && $month_fis_dep > 0) {
                        ...
                        for ($y = $newYearFA; $y <= $yearDepreciation; $y++) { 
                            ...
                            )
                            {
                                ...
                                $realM2 = $dateOldDep ? $this->diffMonths($dateOldDep, $finishDateFA) : 0;
                                $realMonthL = ($dateOldDep && $realM2 > 0 && strtotime($initialDateFA) <= strtotime($dateOldDep)) ? $realM2 : $this->diffMonths($initialDateFA, $finishDateFA);
                                $depreciation = ($updatedValue * $arrayYearSumDigits[$yearFA] / $sumOfDigits) / 12 * $realMonthL; //! Depreciación Gesctión   #####/12 * 12 = 1 toda la gestion
                            ...
                            // echo  '| '.print_r($updatedValue ." : ". $accumDepr, true).' |';
                            // echo  '| '.print_r($fixedAsset["nombre"] ." ::: ". ($yearFA + $newUsefulLife) ." - ". $y, true).' |';
                            ...
                            if ($y == $yearDepreciation) {
                                if ($this->diffMonths($fiscalDateY, $dateDepreciation, false) > 0){
                                     ...
                                     * fecha de compra del activo o de toda la gestion
                                     * */ 
                                    if($this->diffMonths($newDateFA, $fiscalDateY) > 0 && $this->diffMonths($newDateFA, $fiscalDateY) <= 12) {
                                        $realM1 = $this->diffMonths($initialDateFA, $fiscalDateY);
                                        $update = $revaluation == 0 ? (floatval($incomeValue) - $salvageValue) * $updateCoefficient : $revaluation * $updateCoefficient;
                                        $updatedValue =  $revaluation == 0 ? (floatval($incomeValue) - $salvageValue) + $update : $revaluation + $update;
                                        if ($existUsefulLife) {
                                            $depreciation = ($updatedValue * $arrayYearSumDigits[$y] / $sumOfDigits) / 12 * ($realM1); //! Depreciación Gesctión   #####/12 * # = # solo desde la compra del activo
                                        }
                                    // Calcula los datos de gestiones anterioes a la gestión actual de la iteración.
                                    } else {
                                        $update = $updatedValueOld * $updateCoefficient;
                                ...
                                if ($existUsefulLife) {
                                    $testRM = $dateOldDep ? $this->diffMonths($dateOldDep, $dateDepreciation, false) : -1;
                                    $realMonth = $testRM >= 0 && $testRM < 12 ? $testRM : $this->diffMonths($initialDateFA, $dateDepreciation, false);
                                    $depreciation = ($updatedValue * $arrayYearSumDigits[$y] / $sumOfDigits) / 12 * $realMonth; //! Depreciación Gesctión   #####/12 * 12 = 1 toda la gestion
                     ...
                     */
                        ...
                        if ($existUsefulLife) {
                            ...
                            //! Depreciación Gesctión   #####/12 * #:  # = solo desde la (compra, revaluo, dato anterior) del activo
                            $depreciation = ($updatedValue * $arrayYearSumDigits[$newYearFA] / $sumOfDigits) / 12 * $m; 
                    ...
                    if (!($this->gtNW || $this->stNW) || $showData === TRUE) {
                        ...
                        $partialAccumDeprU += round((float)$accumDeprU, 2);
                        $partialAccumDepr += round((float)$accumDepr, 2);
                        $partialNetWorth += (float)round(($updatedValue - $accumDepr), 2) == 0 ? 1 : round((float)($updatedValue - $accumDepr), 2);
                        ...
                        $totalAccumDeprU += round((float)$accumDeprU, 2);
                        $totalAccumDepr += round((float)$accumDepr, 2);
                        $totalNetWorth += (float)round(($updatedValue - $accumDepr), 2) == 0 ? 1 : round((float)($updatedValue - $accumDepr), 2);
        ...
        foreach ($categories as $keyC => $category) {
            ...
            for ($i=$fa_array_position; $i < sizeof($fixedAssets); $i++) { 
                ...
                if($fixedAsset["categorias_id"] ==  $category["id"]){
                    ...
                    // Se obtiene el año fiscal para la fech de compra del activo fijo.
                    $minYear = $yearFA < $yearDepreciation ? $yearFA : $yearDepreciation;
                    $test_fiscal_date = $minYear ."-". $fiscalPeriod;
                    $year_fiscal_p = $this->diffMonths($dateFA, $test_fiscal_date) > 0 ? $minYear : $minYear + 1;
                    $fiscalDate = $year_fiscal_p ."-". $fiscalPeriod;
                    $month_fa_f= $this->diffMonths($dateFA, $fiscalDate);
                    ...
                    if (!($this->gtNW || $this->stNW) || $showData === TRUE) {
                        ...
                        $partialAccumDeprU += round((float)$accumDeprU, 2);
                        $partialAccumDepr += round((float)$accumDepr, 2);
                        $partialNetWorth += round((float)($updatedValue - $accumDepr), 2);
                        ...
                        $totalAccumDeprU += round((float)$accumDeprU, 2);
                        $totalAccumDepr += round((float)$accumDepr, 2);
                        $totalNetWorth += round((float)($updatedValue - $accumDepr), 2);
    ...
    public function StraightLineChart($data, $previousData = []){ 
        ...
        foreach ($categories as $keyC => $category) {
            ...
            $arrayCAtegoryValue[$category["nombre"]] = round((float)$partialNetWorth, 2);
        }
    ...
    public function BarChart($data){ 
        ...
        foreach ($categories as $keyC => $category) {
            ...
            $arrayCAtegoryValue[$category["nombre"]] = round((float)$partialValue, 2);
        }
        return $arrayCAtegoryValue;
    }
            ...
            for ($i=$fa_array_position; $i < sizeof($fixedAssets); $i++) { 
                ...
                if($fixedAsset["categorias_id"] ==  $category["id"]){
                    ...
                    <tr class="afr-tr-e">
                        ...
                        <td>'. $pdf->FormatoNumber($accumDeprU) .'</td>
                        <td>'. $pdf->FormatoNumber($accumDepr) .'</td>
                        <td>'. ( (float)round(($updatedValue - $accumDepr), 2) == 0 ? 1 : $pdf->FormatoNumber($updatedValue - $accumDepr) ) .'</td>
                    </tr>';
                    $partialPrice += $isRevaluation ? 0.00 : floatval($incomeValue);
                    $partialSalvage += $isRevaluation ? 0.00 : round((float)$salvageValue, 2);
                    $partialRevaluation += round((float)$revaluation, 2);
        ...
        $tableHeader = '<div>
            ...
            <table class="afr-table-dpr" >
                <thead>
                    ...
                    <tr>
                        <th >'. date('d/m/Y', strtotime($previousDateF)) .'</th>
                    </tr>
                    <tr >
                        <td class="afr-void" colspan="18"></td>
                    </tr>
    ...
    public function StraightLineFA($data, $previousData = [], $extra = null){ 
        $pdf = new PDFFile();

        $salePrice = $extra["venta_baja"];
        ...
        $quantity = (!empty($extra["cantidad"]) && intval($extra["cantidad"]) >= 1) ? intval($extra["cantidad"]) : intval($fixedAsset["cantidad"]);
        ...
        if ($previousData) {
            ...
            foreach ($previousData as $key => $value) {
                ...
                $tbodyDepreciation .= '<tr class="afr-tr-e">
                    ...
                    <td >'. $pdf->FormatoNumber($accumDeprU) .'</td>
                    <td >'. $pdf->FormatoNumber($accumDepr) .'</td>
                    <td >'. $pdf->FormatoNumber($netWorth) .'</td>
                </tr>';
        ...
        $faQuantity = intval($fixedAsset["cantidad"]) > 1 ? intval($fixedAsset["cantidad"]) : 1;
        $totalReal = ((($lastNetWorth /  $faQuantity) * $quantity) - $baja) * -1;
        $htmlRowTotal = '
            <tr class="afr-tr-e">
                <td colspan="11"> Valor venta o baja (Cantidad: '. $quantity.', Nombre: '. $fixedAsset["nombre"] .' - '. $fixedAsset["detalle"] .'): </td>
                <td >'. $pdf->FormatoNumber($baja) .'</td>
            </tr>
            <tr class="afr-tr-e">
                <td colspan="11">'. $perdidaGanancia .' en venta/baja de Activos Fijos: </td>
                <td >'. $pdf->FormatoNumber($totalReal) .'</td>
            </tr>';
        ...
        $tableHeader = '<div>
            ...
            <div class="afr-info">
                <div class="afr-info-left">
                    ...
                    <div><span>Cantidad:</span> '. $fixedAsset["cantidad"] .'</div>
    ...
    public function SumOfDigitsFA($data, $previousData = [], $extra = null){ 
    ...
    public function ProductionUnitsFA($data, $previousData = [], $extra = null){ 
        $pdf = new PDFFile();
        ...
        $htmlRowTotal = '
            <tr class="afr-tr-e">
                <td colspan="14"> Valor venta o baja (Cantidad: '. $quantity.', Nombre: '. $fixedAsset["nombre"] .' - '. $fixedAsset["detalle"] .'): </td>
                <td >'. $pdf->FormatoNumber($baja) .'</td>
            </tr>
            <tr class="afr-tr-e">
                <td colspan="14">'. $perdidaGanancia .' en venta/baja de Activos Fijos: </td>
                <td >'. $pdf->FormatoNumber($totalReal) .'</td>
            </tr>';
