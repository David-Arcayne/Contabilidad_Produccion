...
export const ActivosFijos = (datosVista) => {
    ...
    return ({registro}) => {
        ...
        if (registro.estado == 5 || registro.estado == 6 || registro.estado == 7 || registro.estado == 8) {
            // if (registro.encargado_id) {
            //     formularioEnUso = formularioAFDevolucion(`trabajador/${registro.personal_id}`);
            // }else {
            // }
        ...
        obtenerDatosAlr(`${URL}/${registro.id}`, { contenedor: contenedorDeAlertas, error: "Ocurrio un error al cargar los registros", accion: cargarContenido, tBody });
