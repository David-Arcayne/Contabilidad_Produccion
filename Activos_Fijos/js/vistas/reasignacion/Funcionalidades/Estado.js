            ...
            elemento.addEventListener("click", () => {
                const registrar = async() => {
                    ...
                    if (activosFijos.data && activosFijos.data.length > 0) {
                        ...
                        const opciones = {
                            datos: datos,
                            myUrl: `${URL}/aceptar-todo/${registro.id}/12`,
                ...
                vistaPrincipal.appendChild(modalDeConfirmacion(registrar, "¿Esta seguro de confirmar los movimientos?"));
            });
        } else if (registro.estado == 12 ) {
            const i = crearElemento("i", {class: "bi bi-hand-thumbs-up-fill"});
            elemento.setAttribute("class", "btn btn-sm btn-success pe-none");
            elemento.append(i, " Confirmado");
        }
    }
}
