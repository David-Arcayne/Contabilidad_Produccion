...
export async function stock_material_porcompra(code_, permisos, refrescar) {
    code = code_;
...
const sitio = () => {
    ...
    buscar.addEventListener('click',()=>{
        ...
        // Filtrar la lista
        const materialesFiltrados = Listastockmaterial.filter(material => {
            const fecha = new Date(material.fecha_caducidad); // Convertir la fecha a formato Date
            return fecha >= new Date(fechaInicio) && fecha <= new Date(fechaFin);
        });
        Listar_stock_material(materialesFiltrados);
    });
...
function subtituloreporte(){
    ...
    if(!resultado){
        ...
        return 'Periodo Caducidad: ' + fuG.cambiarFormatoFecha(resultado.fechaInicio) + ' al ' + fuG.cambiarFormatoFecha(resultado.fechaFinal);
    }
}
function generarNombreArchivo() {
  ...
  function combinar_porcompras(lista) {
    // Ordenar la lista por compra_idcompra
    lista.sort((a, b) => a.compra_idcompra - b.compra_idcompra);

    const resultado = [];

    lista.forEach((item) => {
        // Buscar si ya existe un elemento similar en el resultado
        const existente = resultado.find((elemento) => 
            elemento.compra_idcompra === item.compra_idcompra &&
            elemento.costo_unitario === item.costo_unitario &&
            elemento.peso_neto === item.peso_neto &&
            elemento.costo_envase === item.costo_envase &&
            elemento.tipo_envase_idtipo_envase === item.tipo_envase_idtipo_envase &&
            elemento.fecha_caducidad === item.fecha_caducidad &&
            elemento.material_idmaterial === item.material_idmaterial &&
            elemento.codigo_mat === item.codigo_mat &&
            elemento.medida_idmedida === item.medida_idmedida &&
            elemento.proveedor_idproveedor === item.proveedor_idproveedor &&
            elemento.nombre_mat === item.nombre_mat &&
            elemento.nombre_seccion === item.nombre_seccion &&
            elemento.empresa_idempresa === item.empresa_idempresa &&
            elemento.control_calidad_idcontrol_calidad === item.control_calidad_idcontrol_calidad
        );

        if (existente) {
            // Si ya existe, sumar cantidad y cantidad_envases
            existente.cantidad += item.cantidad;
            existente.cantidad_envases += item.cantidad_envases;
           
        } else {
            // Si no existe, agregarlo al resultado
            resultado.push({ ...item });
        }
    });
...
function Listar_stock_material(Listastockmaterial){
    ...
    let view = "", ind = 1;
    let filtrados_porcompras = combinar_porcompras(Listastockmaterial);
    filtrados_porcompras.map(lista => {
        ...
        view += `
            ...
            <tr>
                ...
                <td >${lista.peso_neto} ${medida.nombre}</td> 
