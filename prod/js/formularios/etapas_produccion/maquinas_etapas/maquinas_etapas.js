...
function parametros_extras(item){
  ...
  if(item['entidad'] === 'etapa'){
      ...
      console.log(Lista_etapas_maquinas_registrados);
      let etapa_maquina_SELECT = Lista_etapas_maquinas_registrados.find((obj) =>Number(obj.idetapas_produccion) === Number(item.idetapas_produccion));
...
async function sitio() {
  ...
  app.innerHTML = view;
  listar_rubro();
  initializeDropdownSearch_(codigos.codigo_etapa, Lista_etapas_produccion,'nombre_etapa',true,'etapa');
...
function lista_etapas_produccion_maquina() {
  ...
  Lista_etapas_maquinas.map((lista) => {
    let itemseccion = list_seccion.find((obj) => Number(obj.id) === Number(lista.seccion)) || {nombre_seccion: "-",};
