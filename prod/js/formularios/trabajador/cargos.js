...
export async function cargos_rh_config(code, permisos, refrescar) {
  code_ = code;
  permisos_ = permisos;
...
async function listar() {
  try {
    ...
    Lista_cargos = resultados[1];
    console.log(Lista_cargos);
  } catch (error) {
...
async function eliminar_cargos(idcargos ){
  if (confirm("Desea Eliminar..?")) {
    const data = await listarFunctions.listar_api_general('eliminar_cargos',idcargos);
...
async function toggleEditSave(event){
    const columnas = [
      ...
      const resultado = await Editar_fila_(event, codigo, Lista_cargos, columnas, 'idcargos');
...
async function sitio() {
  ...
  view += `
          <div class="container">
              ...
              <h5 class="text-center mb-4 fw-bold fs-6" >Cargos</h5>
              <form style="display: none; opacity: 0; height: 0; overflow: hidden; transition: height 0.5s ease, opacity 0.5s ease;" id="formulario${codigo}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="cargo" name="cargo" placeholder="Nombre del cargo" required>
                </div>
                <div class="mb-3">
                    <label for="salario" class="form-label">Salario</label>
                    <input type="number" class="form-control" id="salario" name="salario" placeholder="Salario mensual" step="0.01" required>
                </div>
                <div class="mb-3">
                    <label for="descripcion" class="form-label">Descripción</label>
                    <textarea class="form-control" id="descripcion" name="descripcion" rows="3" placeholder="Descripción del cargo" required></textarea>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label for="areas_idareas" class="form-label">Área</label>
                    <select class="form-select" id="areas_idareas${codigo}" name="areas_idareas" required>
            ...
            <div class="mt-4" style = "max-height: 350px; overflow-y: auto; display: block;">
                ...
                <table class="table table-striped table-hover" id = "editableTable${codigo}">
                  ...
                  <tbody id="listado_cargos${codigo}">
        ...
        `;
//listado_areas       listado_cargos       eliminar_cargos       eliminar_cargos
//editar_cargos($idareas,$nombre, $descripcion, $fecha,$sucursal)
//editar_cargos($idcargos,$cargo, $salario, $descripcion, $fecha,$sucursal)
  ...
  forme.addEventListener("submit",async  (e) => {
    ...
    formData.append('ver','registroCargo');
...
function listado_cargos() {
  const table_body = document.getElementById(`listado_cargos${codigo}`);
  let view = "",ind = 1;
  ...
  Lista_cargos.map((lista) => {
    ...
    "cargo": "administrador",
    "salario": "2500",
    "descripcion": "Responsable de administracion de Comercial",
    "fecha": "2023-12-27",
    "areas_idareas": "3"
}
        let area = Lista_Areas.find(obj => Number(obj.idareas) === Number(lista.areas_idareas));
  
      let actualizar = {
        0: ``,
        1: `
              <div class="text-center">
                  <a  data-id="editar_cargos,${lista.idcargos}"
      ...
      let eliminar = {
        ...
        1: `
            <div class="text-center">
                <a  data-id="eliminar_cargos,${lista.idcargos}"
      ...
      view += `
              <tr>
                  <td>${ind++}</td>                
                  <td data-type="${lista.idcargos},cargo,cargo">${lista.cargo}</td>
                  <td data-type="${lista.idcargos},salario,salario">${lista.salario}</td>             
                  <td data-type="${lista.idcargos},descripcion,descripcion">${lista.descripcion}</td>             
                  <td data-type="${lista.idcargos},areas_idareas,areas_idareas">${area.nombre}</td>             
                  <td>
