...
export function rubroconfig(code, permisos, refrescar) {
...
function eliminar_rubro(id,ids){
    if(confirm("Desea Eliminar..?")){
        fetch(`./api/eliminar_rubro/${id}/${ids}`)
        .then(res=>res.json())
        .then(data=>{
...
function sitio(){
    let view=`
        <div class="container">
            <h2 class="text-center">Rubro productivo</h2>
...
function listar_rubro(){
    ...
    fetch(`./api/listar_rubro/${uk[0].empresa.idempresa}`)
    .then(res=>res.json())
    .then(data=>{
