...
export async function registro_limpieza(code, permisos, refrescar) {
  code_ = code;
  permisos_ = permisos;
...
async function listar() {
  try {
    ...
    const resultados = await Promise.all([
      ...
      listarFunctions.listar_api_general_verd('listar_limpieza',idEmpresa),
    ]);
    ...
    Lista_limpieza = resultados[6];
    console.log(Lista_empleados, Lista_tareas_Limpieza,Lista_limpieza);
  } catch (error) {
...
async function eliminar_limpieza(idlimpieza){
  if (confirm("Desea Eliminar..?")) {
    const data = await listarFunctions.listar_api_general_verd('eliminar_limpieza',idlimpieza);
      
      fuG.alertas(data,codigo);
      if(data[0] == "success" ){
        Lista_limpieza = await listarFunctions.listar_api_general_verd('listar_limpieza',uk[0].empresa.idempresa);
        listar_limpieza();
      }
  }
}
async function toggleEditSave(event) {
  ...
  const resultado = await Editar_fila_(event, codigo, Lista_limpieza, columnas, 'idlimpieza');
  ...
  formData.append("verDavid", "editar_limpieza");
...
if(item['entidad'] === 'tarea'){
    document.getElementById(`tarea_limpieza_idtarea_limpieza${codigos.codigoTareas}`).value = item.idtarea_limpieza;
...
async function sitio() {
  ...
  view += `
        <div class="container mt-5">
            <form style="display: none; opacity: 0; height: 0; overflow: hidden; transition: height 0.5s ease, opacity 0.5s ease;" id="formulario${codigo}">
                <input type="hidden" name="verDavid" value="registrar_limpieza">

                <div class="row g-3">
          
                  <div class="col-md-6">
                        <div style="position: relative;" class="col-md-12">
                            <input type="hidden" name="tarea_limpieza_idtarea_limpieza" id="tarea_limpieza_idtarea_limpieza${codigos.codigoTareas}" required>
                  ...
                  <div class="col-md-4">
                   ...
                   <div class="col-md-4">
                            ...
                            <input type="time" step="0.01" class="form-control" id="hora_inicio" name="hora_inicio" >
                  </div>
                  <div class="col-md-4">
            ...
            <div id="alerta${codigo}" class="mt-4"></div>
              <div>
                  ...
                  <table class="table table-striped table-hover" id = "editableTable${codigo}">
                    ...
                    <tbody id="listar_limpieza${codigo}">
  ...
  listar_limpieza();
  initializeDropdownSearch(codigos.codigoTareas, Lista_tareas_Limpieza,'limpieza',true,'tarea');
...
function listar_limpieza(){
  const table_body = document.getElementById(`listar_limpieza${codigo}`);
  let view = "",ind = 1;
  ...
  Lista_limpieza.map((lista) => {
    ...
    let empleado = Lista_empleados.find(obj => Number(obj.idtrabajador) === Number(lista.empleado_idempleado));
      let actualizar = {
        ...
        1: `
              <div class="text-center">
                  <a  data-id="editar_limpieza,${lista.idlimpieza}"
      ...
      let eliminar = {
        ...
        1: `
            <div class="text-center">
                <a  data-id="eliminar_limpieza,${lista.idlimpieza}"
      ...
      view += `
              <tr>
                  <td>${ind++}</td>                
                  <td data-type="${lista.idlimpieza},tarea_limpieza_idtarea_limpieza,tarea_limpieza_idtarea_limpieza">${tarea.limpieza}</td>
                  <td data-type="${lista.idlimpieza},empleado_idempleado,empleado_idempleado">${empleado.nombre} ${empleado.apellido}</td>
                  <td data-type="${lista.idlimpieza},fecha_inicio,fecha_inicio">${lista.fecha_inicio}</td>
                  <td data-type="${lista.idlimpieza},hora_inicio,hora_inicio">${lista.hora_inicio}</td>
                  <td data-type="${lista.idlimpieza},horas,horas">${lista.horas}</td>
                  <td data-type="${lista.idlimpieza},observaciones,observaciones">${lista.observaciones}</td>
