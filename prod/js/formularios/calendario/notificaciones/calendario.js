...
export async function preparaNotificaciones() {
    ...
    Lista_limpieza.map(lista => {
        ...
        const tiempo = 100000;
        if(eshoy(fecha)){
            let notificacion = 'La tarea ' + tarea.limpieza + ' en seccion '+''+seccion.nombre_seccion + '  la proxima limpieza es en la fecha  ' + fecha;
            
            notificaciones.push(notificacion);
        }
        TodasTareas.push(notificaciones);
    })
    Lista_mantenimiento.map(lista => {
        ...
        console.log(fecha);
        if(eshoy(fecha)){
            let notificacion = 'La tarea ' +tarea.mantenimiento + ' en maquina ' +maquina.nombre + ' el proximo mantenimiento es  ' +fecha;
...
function eshoy(fecha){
    const givenDate = new Date(fecha); // Fecha dada
    const today = new Date(); // Fecha actual

    // Normalizamos ambas fechas al inicio del día (ignorando horas, minutos, etc.)
