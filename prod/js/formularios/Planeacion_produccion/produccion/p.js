...
export async function produccion_productos(code,permisos,refrescar,codigo,id,produccion_idproduccion_) {
  code_ = code;
  permisos_ = permisos;
...
async function listar() {
  try {
    ...
    console.log(resultados[2], resultados[4], resultados[7],resultados[9]);
  } catch (error) {
...
function menu(event) {
  ...
  switch (funcion) {
    ...
    case "solicitar_material":
      modal_solicitar_material(id1,id2);
      break;
...
function modal_solicitar_material(idgrupo, id_orden_produccion){
    ...
    const modal = crearModal({
        ...
        footerButtons: [
            ...
            },
            {
                ...
                text: "Cancelar",
                class: "btn-secondary",
                onClick: () => f_mdl_solicitar_material(),
                dismiss: true // Cierra el modal sin ejecutar ninguna acción
            }
        ]
    });
    f_solicitar_material_m(idgrupo, id_orden_produccion);
...
async function f_solicitar_material_m(idgrupo, id_orden_produccion){
  ...
  const materialesUnicos = Object.values(
    materialsolicitado.reduce((acc, item) => {
        ...
        if (!acc[item.material_idmaterial]) {
            ...
            console.log(acc[item.material_idmaterial].cantidad,item.cantidad);
...
async function preparar_registro(materialsolicitado) {
  const [fecha, hora, horaCompleta] = await fecha_and_hora();
  console.log(document.getElementById(`fecha${codigos.codigoSolicitud_material}`).value);
  console.log(document.getElementById(`hora${codigos.codigoSolicitud_material}`).value);
  ...
  let view = "",
    ...
    view += `
            <tr>
                ...
                <td>
                    <a data-id="editar_estandar_producto_material,${lista.iddetalle_solicitud_material}" class="btn btn-primary btn-sm btn${codigos.codigoSolicitud_material}">
...
async function procesarContenido(contenido) {
  ...
  for (const listar of contenido){
      ...
      console.log(Lista_estandar_productos, listar.producto_idproducto);
      ...
      console.log(Lista_estandar_productos);
  }
  console.log(material_solicitado);
...
function existe_solicitud(id_orden_produccion){
  return solicitudes_material.some((obj)=> Number(obj.produccion_idproduccion) === Number(id_orden_produccion));
}
...
async function sitio() {
  ...
  console.log();
  if(existe_solicitud(idorden_produccion)){
    btn_solicitar_material.innerHTML = ``;
  }else{
    btn_solicitar_material.innerHTML = `
      <div class="d-flex gap-3 ms-auto">
            <div class="text-center">
                <a data-id="solicitar_material,0,${idorden_produccion}"
  ...
  Object.keys(Lista_orden_produccion_ordenada_por_grupos).forEach(key => {
    ...
    botones.innerHTML = `
        <div class="d-flex gap-3 ms-auto">
          <div class="text-center">
              <a data-id="comenzar_produccion,${key},${idorden_produccion}"
    ...
    if(grupoNombre !== "Productos sin etapas de producción"){
      ...
      divGrupo.style.backgroundColor = 'rgba(139, 0, 0, 1)';
    }
