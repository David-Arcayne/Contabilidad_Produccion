...
export function materialconfig(code, permisos, refrescar, codigo_) {
...
function getBoliviaDateTime() {
    return fetch('http://worldtimeapi.org/api/timezone/America/La_Paz')
        .then(response => response.json())
        .then(data => {
            console.log(data);
            const boliviaTime = new Date(data.datetime);

            // Obtener la hora en formato HH:MM:SS basado en la configuración de Bolivia
            const hours = String(boliviaTime.getUTCHours() - 4).padStart(2, '0'); // Bolivia es UTC-4
            const minutes = String(boliviaTime.getUTCMinutes()).padStart(2, '0');
            const seconds = String(boliviaTime.getUTCSeconds()).padStart(2, '0');
            const currentTime = `${hours}:${minutes}:${seconds}`;

            // Obtener la fecha en formato YYYY-MM-DD basado en la configuración de Bolivia
        ...
        .catch(error => {
            console.error('Error al obtener la fecha y hora de Bolivia:', error);
            return null;
        });
}
...
function sitio() {
    ...
    let view = `
        <div class="container ">
                     ...
                     <div class="container my-4">
                        <div class="row mt-4">
                            <div class="col-md-6 form-container mt-4">
                                <label for="" class="form-label fw-bold fs-6">Registrar tipo material</label>
            ...
            <button type="button" class="btn btn-outline-warning mt-4" id="toggleButtonTipo${codigo}"><i class="bi bi-plus">Tipo Material</i> </button>
    ...
    tipoMat.style.display = "none";
    tipoMat.style.opacity = 0;
    tipoMat.style.height = "0";
    tipoMat.style.overflow = "hidden";
    tipoMat.style.transition = "height 0.5s ease, opacity 0.5s ease";
    ...
    .then(() => {
        ...
        toggleButton.addEventListener("click", (e) => mostrarFormulario(e,toggleButton,forme,"Material"));

        const toggleButton2 = document.getElementById(`toggleButtonTipo${codigo}`);
        toggleButton2.addEventListener("click", (e) => mostrarFormulario(e,toggleButton2,tipoMat,"Tipo"));
    })
    .catch(error => {
