...
export function medidas_config(code, permisos, refrescar,codigo) {
...
const codigo = Array.from({ length: 5 }, () => rand()).join("") + "producto_unidad";
...
function menuTabla(event){
    ...
    switch (funcion) {
        case "eliminar_unidad_producto":
            

            eliminar_unidad_producto(id1,id2);
            break;
        case "editar_estado_unidad_producto":
            editar_estado_unidad_producto(event);
            break;
        case "editar_unidad_producto":
...
function toggleEditSave(event) {
    ...
    const [funcion, id1, id2] = dataid.split(',');
    const objetoSeleccionado = Lista_Unidad_Producto.find(obj => obj.id === Number(id1));
    ...
    function guardarCambios() {
        ...
        if(linea){
            ...
            if (objetoSeleccionado) {
                Object.assign(Lista_Unidad_Producto, objetoSeleccionado);
            }
            ...
            if (!areObjectsEqual(objetoSeleccionado, objetoSeleccionadoc)) {
                ...
                obt_unidad_aux = { ...obt_unidad_aux, ...objetoSeleccionadoc };
                sendformData(event, formData);
            }

            console.log(Lista_Unidad_Producto);
...
function eliminar_unidad_producto(id,ids){
    if(confirm("Desea Eliminar..?")){
        fetch(`${URL_APIP}api/eliminar_unidad_producto/${id}/${ids}`)
        .then(res=>res.json())
        .then(data=>{
            obt_unidad_aux ={...Lista_Unidad_Producto.find(obj => obj.id === Number(id))};
...
function editar_estado_unidad_producto(event){
    ...
    const dataid = event.currentTarget.getAttribute('data-id');
    const [funcion, id1, id2] = dataid.split(','); 
    let objeto = Lista_Unidad_Producto.find(obj => obj.id === Number(id1));
...
function sitio(){
    ...
    let view=`
        <div class="container">
            ...
            <h5 class="text-center mb-4 fw-bold fs-6" >Medidas</h5>
            <form id="formulario${codigo}" style="display: none; opacity: 0; height: 0; overflow: hidden; transition: height 0.5s ease, opacity 0.5s ease;">
                <input type="hidden" name="ver" value="registrar_unidad_producto">
            ...
            <div class="scrollable-table mt-4" >
                ...
                <table class="table table-hover" id = "editableTable${codigo}">
                    ...
                    <tbody id="listar_unidad_producto">              
                    </tbody>
                </table>
            </div>
        </div>
...
function edit_Celda_table(e){
    const target = e.target;
        if (target.tagName.toLowerCase() === "td" && !target.classList.contains("editing")) {
            ...
            let obj_seleccionado = Lista_Unidad_Producto.find(obj => obj.id === Number(inpId));
            ...
            input.addEventListener("keydown", function(event) {
                if (event.key === "Enter") {
                    ...
                    if (obj_seleccionado) {
                        Object.assign(Lista_Unidad_Producto, obj_seleccionado);
                    }
                    console.log(Lista_Unidad_Producto);
                    console.log(areObjectsEqual(obj_seleccionado,confirm));
                    if(!areObjectsEqual(obj_seleccionado,confirm)){
                        const formData = new FormData();
                        formData.append('ver', "editar_unidad_producto");
                        formData.append('id', inpId);
                        formData.append('empresa', id2);
                        Object.entries(obj_seleccionado).forEach(([key, value]) => {
                            formData.append(key,value);
                        });
                        obt_unidad_aux = { ...obt_unidad_aux, ...confirm };
...
function listar_unidad_producto(){
    
    fetch(`${URL_APIP}api/listar_unidad_producto/${uk[0].empresa.idempresa}`)
    .then(res=>res.json())
    .then(data=>{
        // const jsonString = JSON.stringify(data);
        // localStorage.setItem("clientes", jsonString);
        Lista_Unidad_Producto.length = 0;
        Lista_Unidad_Producto = data;     
        listar_unidad_productoArray();
        
        
    })
}
function listar_unidad_productoArray(){
    const mon=document.querySelector("#listar_unidad_producto");
    let view="",ind=1;
        Lista_Unidad_Producto.map(lista=>{
            console.log(Lista_Unidad_Producto);
            ...
            <tr>
                ...
                <td>
                    <a data-id="editar_estado_unidad_producto,${lista.id},${uk[0].empresa.idempresa}"  class="btn">
                        ${est}
                    </a>
                </td>
                <td data-type="${lista.id},descripcion,${uk[0].empresa.idempresa}">${lista.descripcion}</td>
                <td>
                    <a data-id="editar_unidad_producto,${lista.id},${uk[0].empresa.idempresa}" class="btn btn-primary btn-sm">
                        <i class="bi bi-pencil-square"></i>
                    </a>  
                    <a data-id="eliminar_unidad_producto,${lista.id},${uk[0].empresa.idempresa}" class="btn btn-danger">
...
function alertas(data) {
    ...
    if (data[0] == "ok") {
        ...
        if(data[2]=="editar_estado_unidad_producto"){
            return;
        }
        ...
        if(data[2]==="registrar_unidad_producto"){
            listar_unidad_producto();

        }
        if(data[2]==="eliminar_unidad_producto"){
            console.log(Lista_Unidad_Producto);

            console.log(obt_unidad_aux);
            Lista_Unidad_Producto = Lista_Unidad_Producto.filter(obj => obj.id !== Number(obt_unidad_aux['id']));
            console.log(Lista_Unidad_Producto);
            listar_unidad_productoArray();
            obt_unidad_aux = { ...{
                "id" : -1,
        ...
        if(data[2]==="editar_unidad_producto"){
            obt_unidad_aux = { ...{
                "id" : -1,
                "nombre" : '',
                "descripcion" : '',
        ...
        if(data[0] == "Error"){
            ...
            if(data[2]==="editar_unidad_producto"){
                const obj = Lista_Unidad_Producto.find(ob => ob.id === obt_unidad_aux['id']);

                console.log(obt_unidad_aux);

                console.log(obj);
                if (obt_unidad_aux) {
                    Object.assign(obj, obt_unidad_aux);
                }
                console.log(obt_unidad_aux);
