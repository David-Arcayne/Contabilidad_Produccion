...
export async function control_calidad_compra_encurso(code_, permisos_, refrescar_) {
    code = code_;
    permisos = permisos_;
    ...
    switch (funcion) {
        ...
        case "generar_form_control_calidad":
            if(id4 == "L_Prod"){
                ...
                generar_form_control_calidad(id1,id2,id3);

            }
            break;
        case "actulizar_form_ctr_cal":
            actulizar_form_ctr_cal(id1,id2);
            break;
...
async function Finalizar_para_almacen_producto(idcontrol_calidad,idlote) {
    ...
    modales.crearModalSoS({
        ...
        header: `  
            <div class="container">
                <h6 class="fw-bold text-emphasis mb-2 text-center">Productos Listos para Almacén: Control de Calidad Completado</h6>
            </div>
        `,
        body: `
            <div class="col">
    ...
    async function listar_almacen(idcontrol_calidad){
        ...
        lista_carac_eval.forEach((firstLevel) => {
            ...
            listaAlmacen.push(listarMateriales_devolucion(firstLevel));
...
async function actulizar_form_ctr_cal(iddetalle_control_calidad,idcontrol_calidad) {
    const modalCargando = document.getElementById('modalCargando');
    const spinner = document.getElementById('spinner');
    const mensajeCargando = document.getElementById('mensajeCargando');
    const contenido = document.getElementById('contenido');
...
export async function sitio(){
    ...
    console.log(ctr_calidad);
    ctr_calidad.map(lista=>{
        let Entidad_tipo;
            
            Entidad_tipo = Lista_lote_produccion.find(obj => Number(obj.idlote) === Number(lista.Entidad_id));
            
             

            let itemempleado = Lista_empleados.find(obj => Number(obj.id) === Number(lista.empleado_idempleado)) || {
                "id": 0,
                "nombre": "-",
                "apellido": "-"
            };
                            ...
                            <div class="row align-items-center ">
                                <div class="col-md-6 ">
                                    <ul class="list-unstyled">
                                        ...
                                        <li><strong>Responsable:</strong> ${itemempleado.nombre} ${itemempleado.apellido}</li>
