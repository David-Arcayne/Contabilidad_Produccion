import { edit_table } from "../funciones/edittable.js";
...
export function producto_categoria_config(code, permisos, refrescar,codigo) {
...
function menuTabla(event){
    ...
    switch (funcion) {
        case "eliminar_categoria":
            ...
            eliminar_categoria(id1,id2);
            break;
        case "editar_estado_categoria":
            editar_estado_categoria(event);
            break;
...
function toggleEditSave(event) {
    ...
    const [funcion, id1, id2] = dataid.split(',');
    const objetoSeleccionado = Lista_Categorias.find(obj => obj.id === Number(id1));
    const objetoSeleccionadoc = {...Lista_Categorias.find(obj => obj.id === Number(id1))};
    console.log(funcion, id1, id2);
    let datos = edit_table(event,permisos,Lista_Categorias,codigo,[],["nombre","descripcion"]);
    console.log(datos);
    ...
    //     if(linea){
    //         objetoSeleccionado['nombre'] = datosnuevos[0];
    //         objetoSeleccionado['descripcion'] = datosnuevos[1];
    //         if (objetoSeleccionado) {
    //             Object.assign(Lista_Categorias, objetoSeleccionado);
    //         }
    ...
    //             });
    //             obt_categorias_aux = { ...obt_categorias_aux, ...objetoSeleccionadoc };
...
function eliminar_categoria(id,ids){
    if(confirm("Desea Eliminar..?")){
        fetch(`${URL_APIP}/api/eliminar_categoria/${id}/${ids}`)
        .then(res=>res.json())
        .then(data=>{
            obt_categorias_aux ={...Lista_Categorias.find(obj => obj.id === Number(id))};

            alertas(data);
            console.log(obt_categorias_aux);
...
function editar_estado_categoria(event){
    const boton = event.currentTarget;
    const dataid = event.currentTarget.getAttribute('data-id');
    const [funcion, id1, id2] = dataid.split(','); 
    let objeto = Lista_Categorias.find(obj => obj.id === Number(id1));
...
function sitio(){
    ...
    let view=`
        <div class="container">
            ...
            <h5 class="text-center mb-4 fw-bold fs-6" >Producto-categorias</h5>
            <form id="formulario${codigo}" style="display: none; opacity: 0; height: 0; overflow: hidden; transition: height 0.5s ease, opacity 0.5s ease;">
                <input type="hidden" name="ver" value="registroCategoria">
                <input type="hidden" name="empresa" value="${uk[0].empresa.idempresa}">
                <input type="hidden" class="form-control" id="idp" name="idp" value="0">
                <div class="row">

                    <div class="col-md-6 mb-3">
                        <label for="nombre">Nombre</label>
                        <input type="text" class="form-control ml-4" id="nombre" name="nombre" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="estado">Estado</label>
                        <select class="form-select mr-4" id="estado${codigo}" name="estado">
...
function edit_Celda_table(e){
    const target = e.target;
        if (target.tagName.toLowerCase() === "td" && !target.classList.contains("editing")) {
            ...
            let obj_seleccionado = Lista_Categorias.find(obj => obj.id === Number(inpId));
            ...
            input.addEventListener("keydown", function(event) {
                if (event.key === "Enter") {
                    ...
                    if (obj_seleccionado) {
                        Object.assign(Lista_Categorias, obj_seleccionado);
                    }
                    console.log(Lista_Categorias);
                    console.log(areObjectsEqual(obj_seleccionado,confirm));
                    if(!areObjectsEqual(obj_seleccionado,confirm)){
                        const formData = new FormData();
                        formData.append('ver', "editar_categoria");
                        formData.append('id', inpId);
                        formData.append('empresa', id2);
                        Object.entries(obj_seleccionado).forEach(([key, value]) => {
                            formData.append(key,value);
                        });
                        obt_categorias_aux = { ...obt_categorias_aux, ...confirm };
...
function listarCategorias(){
    ...
    fetch(`${URL_APIP}/api/listarCategorias/${uk[0].empresa.idempresa}`)
    .then(res=>res.json())
    .then(data=>{
        ...
        listarCategoriasArray();
        
        
    })
}
function listarCategoriasArray(){
    const mon=document.querySelector("#listarCategorias");
    let view="",ind=1;
        Lista_Categorias.map(lista=>{
            console.log(Lista_Categorias);
            ...
            <tr>
                ...
                <td>
                    <a data-id="editar_estado_categoria,${lista.id},${uk[0].empresa.idempresa}"  class="btn">
                        ${est}
                    </a>
                </td>
                <td data-type="${lista.id},descripcion,${uk[0].empresa.idempresa}">${lista.descripcion}</td>
                <td>
                    <a data-id="editar_categoria,${lista.id},${uk[0].empresa.idempresa}" class="btn btn-primary btn-sm">
                        <i class="bi bi-pencil-square"></i>
                    </a>  
                    <a data-id="eliminar_categoria,${lista.id},${uk[0].empresa.idempresa}" class="btn btn-danger">
...
function alertas(data) {
    ...
    if (data[0] == "ok") {
        ...
        if(data[2]=="editar_estado_categoria"){
            return;
        }
        ...
        if(data[2]==="registroCategoria"){
            listarCategorias();

        }
        if(data[2]==="eliminar_categoria"){
            console.log(Lista_Categorias);

            console.log(obt_categorias_aux);
            Lista_Categorias = Lista_Categorias.filter(obj => obj.id !== Number(obt_categorias_aux['id']));
            console.log(Lista_Categorias);
            listarCategoriasArray();
            obt_categorias_aux = { ...{
                "id" : -1,
        ...
        if(data[0] == "Error"){
            ...
            if(data[2]==="editar_categoria"){
                const obj = Lista_Categorias.find(ob => ob.id === obt_categorias_aux['id']);

                console.log(obt_categorias_aux);

                console.log(obj);
                if (obt_categorias_aux) {
                    Object.assign(obj, obt_categorias_aux);
                }
                console.log(obt_categorias_aux);
