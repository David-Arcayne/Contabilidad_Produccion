...
async function listar() {
  ...
  function initializeDropdownSearch_m(codigo, Lista_Material,clave, valor,tipo,idEntidad, condicion,entidad) {
  ...
  function parametros_extras(item){
    ...
    if(item['entidad'] === 'riesgo'){
        ...
        //procedimiento = fuG.ordenar_array(procedimiento,'npaso');
        console.log(procedimiento);
        const resultado = {
...
async function sitio(){
    ...
    initializeDropdownSearch_m(codigos.codigoRiesgos, Lista_riesgo,'codigo','descripcion','tipo_variable','idtipo_variable',false,'riesgo');
    ...
    forme.addEventListener("submit", (e)=>{
        ...
        if (recurso) {
            ...
            let aux = {
                ...
                npaso: Lista_procedimientos.length,
 ...
 function prepararLista_enviar(list) {
   ...
   if (lista_preparada.procedimiento && lista_preparada.procedimiento.length > 0) {
     fetch(`${URL_APIP}api/`, {
       ...
       .then((data) => {
         console.log(data);
         fuG.alerta_success(code,data[0],data[1]);
...
function listar_procedimiento() {
    ...
    let view = "", ind = 1;
    console.log(Lista_procedimientos);
    Lista_procedimientos = fuG.ordenar_array(Lista_procedimientos,'npaso');
    ...
    listar.querySelectorAll(".btn-up").forEach((button) => {
        button.addEventListener("click", () => moverFila(button, "arriba"));
      });
    
    listar.querySelectorAll(".btn-down").forEach((button) => {
