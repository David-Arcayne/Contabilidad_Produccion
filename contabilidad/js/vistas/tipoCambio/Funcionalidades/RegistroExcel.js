...
export const RegistroExcel = (datosVista) => {
    ...
    boton.addEventListener("click", () => {
        ...
        const modalImportarExcel = () => {
            ...
            // Función para verificar si se enviaron todos los datos a la API y mostrar su respectiva alerta
            function enviarDatosALaAPI() {
                let intervalo = setInterval(async function() {
                    if (enviados[2] === enviados[0]) {
                        if (enviados[0] == enviados[1]) {
                            const listaDeRegistros = await obtenerDatos(`${URL}listatipodecambio/${empresa_id}`);
            ...
            async function leerArchivoExcel(archivo) {
                ...
                if (extension ==="csv") {
                    ...
                    lector.onload = async function(evento) {
                        ...
                        let envioLotes = 0;
                        // Recorrer las filas del archivo y enviar los datos a la API
            ...
            // Evento para obtener el archivo seleccionado y leerlo al enviar el formulario
