...
import { btnNuevoRegistro, BuscarEnTablaOpts, cambiarVista, crearElemento, InputBusquedaPlus, opciones } from "../../funciones/Funciones.js";
...
import { FacturasCobrar } from "./index1.js";
import { MenuOtros } from "./index2.js";
 ...
 */
export function SMFacturasCobrar(codigo, permisos) {
    ...
    const vistaFacturas = document.querySelector(`.p-2[data-value="${codigo}"] .card-body #contenedor-formulario`);
    vistaFacturas.setAttribute("class", "d-none");
    const vistaOtros = crearElemento("div", { class: "d-none" });
   
    const contenedorPrincipal = vistaPrincipal.parentNode;
    contenedorPrincipal.append(vistaOtros);

    const textoF = crearElemento("span", { class: "text-center fs-6" }, ["Cobrar Facturas"]);
    const btnFacturas = crearElemento("button", { class: "btn btn-primary me-2 p-3", style: "max-width: 200px; width: 100%;", id: "btn-facturas" }, [textoF]);
    const textoO = crearElemento("span", { class: "text-center fs-6" }, ["Otras Cuentas"]);
    const btnOtros = crearElemento("button", { class: "btn btn-primary", style: "max-width: 200px; width: 100%;",  id: "btn-otros" }, [textoO]);

    const divBtns = crearElemento("div", { class: "d-flex justify-content-center mt-5 pt-5" }, [btnFacturas, btnOtros]);

    btnFacturas.addEventListener("click", () => {
        cambiarVista(vistaPrincipal, vistaFacturas);
    });
    btnOtros.addEventListener("click", () => {
        cambiarVista(vistaPrincipal, vistaOtros);
    });

    vistaPrincipal.append(divBtns);
    ...
    FacturasCobrar(codigo, permisos, vistaFacturas, vistaPrincipal);
    MenuOtros(codigo, permisos, vistaOtros, vistaPrincipal);
}
...
export const InputsFecha = (tabla) => {
    ...
    const divFiltros = crearElemento("div", {class: "row d-flex justify-content-end pb-2 g-2"}, [row, elementoBuscar]);
