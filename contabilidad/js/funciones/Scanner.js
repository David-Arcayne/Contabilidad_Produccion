...
/**
 * Crea un modal para el escáner de código QR.
...
/**
 * Implementa el escáner de código QR en un modal.
 * @param {HTMLDivElement} modalBody - Cuerpo del modal.
 * @param {string} idPantalla - ID del contenedor del escáner.
 * @param {function} accion - Función a ejecutar al leer el código QR.
 * @param {object} opciones - Opciones para la función.
 * @param {string} opciones.tipo - Tipo de acción a realizar.
 */
export const  ImplementarScanner = (modalBody, idPantalla, accion, opciones) => {
    ...
    const contenedorInformacion = crearElemento("div", {class: "py-2 text-start d-none"});
    opciones["resultado"] = contenedorInformacion;
    ...
    // Crear botones de control del escáner
    ...
    // Funcion para cambiar estilos de pantalla según el tipo de contenido
    ...
    inputArchivo.addEventListener("change", (event) => {
        ...
        .then(qrCodeMessage => {
            alertaInf.innerHTML = "";
            alertaHeader.innerHTML = "";
            if (opciones.tipo === "busqueda") {
                accion(qrCodeMessage, opciones);
                const modal = bootstrap.Modal.getInstance(modalScanner);
                modal.hide();
            } else if (opciones.tipo === "informacion") {
                accion(qrCodeMessage, opciones, modalScanner);
            }
        })
        .catch(err => {
    ...
    // Función a ejecutar al leer el código QR correctamente y enviar el resultado a la función "accion"
    const codigoQrExitoso = (decodedText, decodedResult) => {
        ...
        if (opciones.tipo === "busqueda") {
            accion(decodedText, opciones);
            const modal = bootstrap.Modal.getInstance(modalScanner);
            modal.hide();
        } else if (opciones.tipo === "informacion") {
            accion(decodedText, opciones, modalScanner);
        }
    };
    ...
    const iniciarEscaneoQR = (cambiar = false) => {
        ...
        // Obtener cámaras disponibles y empezar a escanear
    ...
    // Detener el escáner de código QR y limpiar los mensajes de alerta y volver a iniciar el escaneo si se desea
...
//     const modalBody = crearElemento("div", {class: "modal-body text-center pt-1", id: "modalBodyScanner"});
//     modalBody.innerHTML = `<div style="width: 400px; height:400px" id="contenedor_htmlqrcode"></div>`;
//     const modalContent = crearElemento("div", {class: "modal-content"}, [modalBody]);
//     const modalDialog = crearElemento("div", {class: "modal-dialog modal-dialog-centered modal-dialog-scrollable"}, [modalContent]);
//     const modal = crearElemento("div", {class: "modal fade", tabindex: "-1", "aria-labelledby": "modalScannerLabel", "aria-hidden": "true"}, [modalDialog]);
//     return modal;
// }