 ...
 * @returns {HTMLElement}
 */
export const modalDeInformacion = (texto) => {
    const informacion = crearElemento("p", {class: "fs-5 text-secondary"}, [texto]);
    const btnContinuar = crearElemento("button", {class: "btn btn-info px-md-4"}, ["Cerrar"]);
    const contenedorMensaje = crearElemento("div", {class: "floating-message text-center p-md-4"}, [informacion, btnContinuar]);
    const contenedorModal = crearElemento("div", {class: "inner-div"}, [contenedorMensaje]);

    // Elimina el modal si se hace click fuera del contendor de mensaje
    contenedorModal.addEventListener("click", function (event) {
        if ((contenedorMensaje && !contenedorMensaje.contains(event.target)) || (btnContinuar && btnContinuar.contains(event.target))) {
            contenedorModal.remove();
        }
    });
...
/**
 * Crea un modal para insertar un formulario (modal removible)
 * @returns {[HTMLElement, HTMLElement]} - Contenedor del modal y el cuerpo del modal
 */
export const modalFormulario = () => {
    const iconoCerrar = crearElemento("i", {class: "bi bi-x-lg"});
    const btnCerrar = crearElemento("button", {class: "btn btn-light"}, [iconoCerrar]);
    const div = crearElemento("div", {class: "text-end mb-4" }, [btnCerrar]);
    const divCuerpo = crearElemento("div", {class: ""});
    const contenidoModal = crearElemento("div", {class: "floating-message p-md-4 overflow-auto", style: "width: calc(100% - 20px); max-height: calc(100% - 20px)" }, [div, divCuerpo]);
    ...
    return [contenedorModal, divCuerpo];
}

/**
 * Crea un modal para insertar un formulario (modal fijo)
 * @returns {[HTMLElement, HTMLElement]} - Contenedor del modal y el cuerpo del modal
 */
export const modalFormularioFijo = () => {
    const iconoCerrar = crearElemento("i", {class: "bi bi-x-lg"});
    const btnCerrar = crearElemento("button", {class: "btn btn-light"}, [iconoCerrar]);
    const div = crearElemento("div", {class: "text-end mb-4" }, [btnCerrar]);
    const divCuerpo = crearElemento("div", {class: ""});
    const contenidoModal = crearElemento("div", {class: "floating-message p-md-4 overflow-auto", style: "width: calc(100% - 20px); max-height: calc(100% - 20px)" }, [div, divCuerpo]);
    const contenedorModal = crearElemento("div", {class: "inner-div d-none" }, [contenidoModal]);
