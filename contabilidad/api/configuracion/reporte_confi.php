...
class Reporte_confi extends DB{
    ...
    public function registrar_tipo_reportes($nombre, $descripcion, $tipo_reporte,$id_plantilla_reporte, $empresa) {
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);
 ...
 private function reporte_balance_general($idplantilla_reporte,$fecha_ini,$fecha_fin,$empresa,$gestion) {
        ...
        // preguntar si la cuenta en la q estamos es activo fijo
            ...
            //    if($nivel_reg['nivel_registrado'] == '4'){
                    ...
                    "profundidad" => '1',
                    "orden" => $qwe2['orden'],
                    "nombre_cuenta_superior" => $qwe2['nombre_cuenta_superior'],
                    "nivel_2" => [] //activo
                    );
                $suma_nivel_2 = 0;
            ...
            //    if($nivel_reg['nivel_registrado'] == '4'){
                ...
                while ($qwe3 = $this->dbc->fetch($get_nivel_3)) {
                    ...
                    if($qwe3['es_calculable'] == 'si'){ // ES CALCULABLE
                        ...
                        $total_otro_reporte = $miObjeto->reporte_calculo_otro_reporte(
    ...
    private function reporte_balance_general_consolidado($idplantilla_reporte,$fecha_ini,$fecha_fin,$empresa,$gestion) {
        ...
        // preguntar si la cuenta en la q estamos es activo fijo
            ...
            //    if($nivel_reg['nivel_registrado'] == '4'){
                    ...
                    $res2 = array(
                    "idconfiguracion_reporte" => $qwe2['idconfiguracion_reporte'],
                    "idplantilla_reporte" => $qwe2['idplantilla_reporte'],
                    "grupo" => $qwe2['grupo'],
                    "negrilla_cursiva" => $qwe2['negrilla_cursiva'],
                    "es_calculable" => $qwe2['es_calculable'],
                    "idplandecuenta" => $nombre_cuenta['idplandecuenta'],
...
//(elimina la cuenta principal, entonces se eliminara ambas cuetas de la tabla confi_reporte y la vinculacion en la tabla vincu)
    ...
    public function registrar_vinculacion_depreciacion($idcuenta,$iddepreciacion,$idtipo_reportes,$empresa,$idgestion){
        // $idempresa = Empresa::getidempresa($empresa);
        $idempresa = $this->getidempresa($empresa);
    ...
    public function guardar_balance_general_por_gestion($data, $nivel = 1, $empresa = null, $idempresa = null, $idgestion = null)
{
    ...
    foreach ($items as $item) {
        ...
        $grupo = $item['grupo'] ?? null;
        $es_calculable = $item['es_calculable'] ?? null;
        $tipo_operacion = $item['tipo_operacion'] ?? null;
        $orden = $item['orden'] ?? null;
...
public function listar_balance_general_completo_icono($idgestion) {
        ...
        while ($qwe2 = $this->dbc->fetch($get_nivel_2)) {
            ...
            $get_nivel_3 = $this->dbc->query("SELECT * FROM balance_general_por_gestion WHERE idgestion = '$idgestion' 
  AND nombre_cuenta_superior ='$qwe2[nombre_actual]' AND grupo ='$qwe2[grupo]'");// ACTIVO, PASIVO, PATRIMONIO
            while ($qwe3 = $this->dbc->fetch($get_nivel_3)) {
                ...
                // "estado" => $qwe['estado']
                 ...
                 $get_nivel_4 = $this->dbc->query("SELECT * FROM balance_general_por_gestion WHERE idgestion = '$idgestion' 
  -- AND nivel ='4' 
    AND nombre_cuenta_superior ='$qwe3[nombre_actual]' AND grupo ='$qwe3[grupo]'
    ORDER BY orden ASC");// ACTIVO, PASIVO, PATRIMONIO
                ...
                "valor" => $qwe4['valor'],
                    ...
                    $get_nivel_5 = $this->dbc->query("SELECT * FROM balance_general_por_gestion WHERE idgestion = '$idgestion' 
  -- AND nivel ='4' 
    AND nombre_cuenta_superior ='$qwe4[nombre_actual]' AND grupo ='$qwe4[grupo]'
    ORDER BY orden ASC");// ACTIVO, PASIVO, PATRIMONIO
                    while ($qwe5 = $this->dbc->fetch($get_nivel_5)) {
                ...
                //----------------------------------------------------------------------------
                    $get_nivel_6 = $this->dbc->query("SELECT * FROM balance_general_por_gestion WHERE idgestion = '$idgestion' 
  -- AND nivel ='4' 
    AND nombre_cuenta_superior ='$qwe5[nombre_actual]' AND grupo ='$qwe5[grupo]'
    ORDER BY orden ASC");// ACTIVO, PASIVO, PATRIMONIO
                    while ($qwe6 = $this->dbc->fetch($get_nivel_6)) {
    ...
    public function reporte_balance_general_por_niveles_consolidados_hasta(
    $idplantilla_reporte,
    $fecha_fin,
    $empresa,
    $maxProfundidad,
    ...
    $array = $this->reporte_balance_general_consolidado_hasta(
        $idplantilla_reporte,
        $fecha_fin,
        $empresa,
        $gestion
    ...
    private function reporte_balance_general_consolidado_hasta($idplantilla_reporte,$fecha_fin,$empresa,$gestion) {
        // ini_set('display_errors', 1); 
        // ini_set('display_startup_errors', 1);
        // error_reporting(E_ALL);
                         ...
                         if($qwe4['es_calculable'] == 'si'){ // ES CALCULABLE
                            ...
                            if($qwe4['es_activo_fijo'] == 'si'){ //ES ACTIVO FIJO
                              ...
                              if($get_fijo->num_rows > 0){
                                ...
                                INNER JOIN plandecuenta p on p.idplandecuenta=dt.idplandecuenta
                                where t.organizacion_idorganizacion='$idempresa' and t.idgestion = '$gestion' and p.idplandecuenta = '$qwe4[idplandecuenta]'
                                AND t.estado NOT IN (4, 5, 6) AND t.consolidar = 2 AND t.fechatransaccion<='$fecha_fin'");
            ...
            //    if($nivel_reg['nivel_registrado'] == '4'){
                ...
                while ($qwe3 = $this->dbc->fetch($get_nivel_3)) {
                    ...
                    if($qwe3['es_calculable'] == 'si'){ // ES CALCULABLE
                        ...
                        // IR AL OTRO REPORTE PARA OBTENER LO QUE QUIERO
                        $miObjeto = new PlantillaReporte();
                        $total_otro_reporte = $miObjeto->reporte_calculo_otro_reporte_consolidado_hasta(
                        $calc_otr_rep['idtipo_reporte_referencia'],   // REPORTE DE REFERENCIA
                        $fecha_fin,
                        $empresa,
                        $gestion
    ...
    public function getgestionactualid($idempresa)
    {

        $res = "";
        $registro = $this->dbc->query("select * from gestion where idempresa='$idempresa' and estado='2' Limit 1");
        $qwe = $this->dbc->fetch($registro);
        //$res=array("id"=>,"nombre"=>$qwe['nombre']); listapagarfactura 
        return $qwe['idgestion'];
    }
//activo--1  getgestionactual  pasivo --2  patrimonio---3    ingresos---4   egresos_gastos --5  orden ---6  eliminar    editar  reporte_balance_general_consolidado listar_configuracion_reporte
}
?>
