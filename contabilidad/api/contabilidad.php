...
WHERE md5(p.organizacion_idorganizacion)='$ide'");
    ...
    public function listatransaciones($empresa)
    {
        $lista = [];
        // 
    ...
    // public function detalletransaccionnormal($trans, $plandecuenta, $debe, $haber, $nota, $empresa, $sucursal)
    // {
    //     $idsucursal = $this->getidsucursal($sucursal);
    //     $ide = $this->getidempresa($empresa);
    ...
    //     $ppresupuestario = 0;
    //     $res = "";
    //     $crear = $this->dbc->query("insert into detalletransaccion(iddetalletransaccion,debe,haber,nota,transacciones_idtransacciones,idplandecuenta,idcuentapresupuestaria,estado,cobrar,pagar,idorganizacion,idsucursal)value(NULL,'$debe','$haber','$nota','$trans','$plandecuenta','$ppresupuestario','$estado','2','2','$ide','$idsucursal')");
    //     if ($crear === TRUE) {
    //         $res = array("success", "Se Registro Correctamente", "detalletransaccionnormal", $trans);
    //     } else {
    //         $res = array("danger", "Lo siento hubo un problema,por favor vuelva a intentar mas tarde");
    //     }
    //     echo json_encode($res);
...
// -----------------------------------------------------------------------------------
    ...
    public function crearsolofacturasapi($fecha, $nfactura, $nautorizacion, $codigocontrol, $monto, $tasacero, $export, $npoliza, $ice, $descuento,$clasefactura,$cobro, $pagar, $espesificacion,$trans, $cliente, $empresa,   $cuenta,  $sucursal)
    {
        $idsucursal = $this->getidsucursal($sucursal);
        $idempresa = $this->getidempresa($empresa);
        $co = 0;
...
WHERE
  ...
  AND asiento.idcuenta = plandecuenta.idplandecuenta;
    ...
    public function lista_cobrar_cobrado_factura($sucursal)
    {
        ...
        $cf = 2;
        $idsucursal = $this->getidsucursal($sucursal);
        $registro = $this->dbc->query("SELECT f.idfactura,f.fecha,f.nfactura,t.codigotransaccion, f.montofactura,f.proveedorcliente_idproveedorcliente,f.transacciones_idtransacciones,f.cuenta,f.cobrado
         FROM factura f,transacciones t
        ...
        while ($qwe = $this->dbc->fetch($registro)) {
            ...
            if($hayIndividuales > 0){
               ...
               $res = array("id" => $qwe[0], "fecha" => $qwe[1], "numero" => $qwe[2], "codigo" => $qwe[3], "idproveedor" => $qwe[5], "nombrep" => $pro['nombre'], "monto" => $qwe[4], "pagado" => $resultado33['monto'], "saldo" => 0, "transaccion" => $qwe[6], "cuenta" => $qwe[7], "detalle" => $pagados);
               array_push($lista, $res);
        ...
        if($hayIndividuales > 0){
            if($hayGrupales > 0){
                // hay grupales e individuales
                        ...
                        while ($zxc = $this->dbc->fetch($mostrarIndi)) {
                            $res = array("recibo" => $zxc[0], "fecha" => $zxc[1], "persona" => $zxc[2], "ci" => $zxc[3], "monto" => $zxc[4], "id" => $zxc[5]);
                            array_push($lista, $res);
                        }
                ...
                while ($qwe = $this->dbc->fetch($registro)) {
                    ...
                    array_push($lista, $res);
                       ...
                       while ($zxc = $this->dbc->fetch($datosRecibo)) {
                           $res = array("recibo" => $zxc[0], "fecha" => $zxc[1], "persona" => $zxc[2], "ci" => $zxc[3], "monto" => $resultado3['monto'], "id" => $zxc[5]);
                           array_push($lista, $res);
                       }
                    //    $res = array("id" => $qwe[0], "fecha" => $qwe[1], "numero" => $qwe[2], "codigo" => $qwe[3], "idproveedor" => $qwe[5], "nombrep" => $pro['nombre'], "monto" => $qwe[4], "pagado" => $resultado3['monto'], "saldo" => 0, "transaccion" => $qwe[6], "cuenta" => $qwe[7], "detalle" => $pagados);
