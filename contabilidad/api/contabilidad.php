...
// -----------------------------------------------------------------------------------
    ...
    public function crearfacturasf5($id, $fecha, $nfactura, $nautorizacion, $codigocontrol, $monto, $tasacero, $export, $npoliza, $ice, $descuento, $cliente,$concepto)
    {
        //$idsucursal=$this->getidsucursal($sucursal); crearfacturasf5
    ...
    public function eliminarfactura($idfactura)
    {
      ...
      public function eliminar_factura_venta_desde_transaccion($idfactura)
    {
        $res = "";
        $existe = $this->dbc->query("SELECT * FROM cuentaspof WHERE idfactura='$idfactura'");
        if($existe->num_rows > 0){
            //NO SE PODRA ELIMINAR PORQUE EXISTEN RECIBOS DENTRO DE ESTA FACTURA
            $res = array("danger", "No se pudo realizar el registro");

        }else{
            //SI SE ELIMINARA PORQUE ES UNA FACTURA SIN NADA DENTRO
            $delete = $this->dbc->query("DELETE FROM factura WHERE idfactura='$idfactura'");
        }
    ...
    public function eliminar_factura_compra_desde_transaccion($idfactura)
    {
        $res = "";
        $existe = $this->dbc->query("SELECT * FROM cuentaspor WHERE idfactura='$idfactura'");
        if($existe->num_rows > 0){
            //NO SE PODRA ELIMINAR PORQUE EXISTEN RECIBOS DENTRO DE ESTA FACTURA
            $res = array("danger", "No se pudo realizar el registro");
      ...
      public function listafactura_cobro_trans($idtransaccion)
    {
        ...
        $factura_conta = $this->dbc->query("SELECT f.idfactura, f.fecha, f.nfactura, f.montofactura, f.clasefactura, f.cobrado, f.pagado, f.estado, f.transacciones_idtransacciones, f.proveedorcliente_idproveedorcliente, f.idorganizacion, f.registro_desde,
        f.nautorizacion, f.codigocontrol, f.tasa0, f.export, f.npoliza, f.iceiecdhotros, f.descuentobonificacion, f.por_concepto_de
        FROM factura  AS f WHERE transacciones_idtransacciones='$idtransaccion' AND f.clasefactura = '2' ORDER BY f.nfactura DESC");

        if($factura_conta->num_rows > 0){
            while ($qwe = $this->dbc->fetch($factura_conta)) {
                if ($qwe['clasefactura'] == 2) {
                    ...
                    // $res = array("id" => $qwe['idfactura'], "fecha" => $qwe['fecha'], "nfactura" => $qwe['nfactura'], "montofactura" => $qwe['montofactura'], "clasefactura" => $qwe['clasefactura'], "cobrado" => $qwe['cobrado'], "pagado" => $qwe['pagado'], "idtransaccion" => $qwe['transacciones_idtransacciones'], "idcliente_proveedor" => $qwe['proveedorcliente_idproveedorcliente'], "procli" => $asd['nombre'], "nit" => $asd['nit'],"desde" => 'contabilidad',"por_concepto_de" => $qwe['por_concepto_de'],"registro_desde" => $qwe['registro_desde']);
                    $res = array("id" => $qwe['idfactura'], "fecha" => $qwe['fecha'], "nfactura" => $qwe['nfactura'],"nautorizacion" => $qwe['nautorizacion'], "codigocontrol" => $qwe['codigocontrol'], "montofactura" => $qwe['montofactura'],"tasacero" => $qwe['tasa0'], "export" => $qwe['export'], "npoliza" => $qwe['npoliza'], "ice" => $qwe['iceiecdhotros'], "descuentobonificacion" => $qwe['descuentobonificacion'], "clasefactura" => $qwe['clasefactura'], "cobrado" => $qwe['cobrado'], "pagado" => $qwe['pagado'],"estado" => $qwe['estado'], "idtransaccion" => $qwe['transacciones_idtransacciones'], "idcliente_proveedor" => $qwe['proveedorcliente_idproveedorcliente'], "procli" => $asd['nombre'], "nit" => $asd['nit'],"desde" => 'contabilidad',"por_concepto_de" => $qwe['por_concepto_de'],"registro_desde" => $qwe['registro_desde']);
...
//----------------------------------------------------------------------------------------------------------------------------------------
    ...
    public function listafactura_pago_trans($idtransaccion) // COMPRAS
    {
        $lista = [];
        $res = "";
  ...
  AND asiento.idcuenta = plandecuenta.idplandecuenta;
    ...
    public function lista_cobrar_cobrado_factura($sucursal)
    {
        ...
        while ($qwe = $this->dbc->fetch($registro)) {
            ...
            $proveedor = $this->dbcm->query("SELECT * FROM cliente WHERE id_cliente='" . $qwe[4] . "'");
    ...
    public function listapagos($idfactura)
    {
        ...
        if($hayIndividuales > 0){
            if($hayGrupales > 0){
                // hay grupales e individuales
                        $mostrarIndi = $this->dbc->query("SELECT nrecibo,fecha,persona,ci,monto,idcuentaspof,transaccion,archivo,lugar,concepto,estado,registro_desde 
                        ...
                        while ($zxc = $this->dbc->fetch($mostrarIndi)) {
                            ...
                            $res = array("recibo" => $zxc[0], "fecha" => $zxc[1], "persona" => $zxc[2], "ci" => $zxc[3], "monto" => $zxc[4], "id" => $zxc[5],"transaccion" => $zxc[6],"nombre_archivo" => $zxc[7],"lugar" => $zxc[8],"nit" => $cl['nit'],"direccion" => $cl['direccion'],"concepto" => $zxc['concepto'],"estado_documento" => $estado_documento,"registro_desde" => $zxc['registro_desde']);
                            array_push($lista, $res);
                        }
                ...
                $resultado33 = $listaGrup2->fetch_assoc();
                $idrecibo2 = $resultado33['idcuentaspof'];
                $datosRecibo2 = $this->dbc->query("SELECT nrecibo,fecha,persona,ci,monto,idcuentaspof,transaccion,archivo,lugar,concepto,estado,registro_desde FROM cuentaspof WHERE idcuentaspof='$idrecibo2'");
                ...
                while ($www = $this->dbc->fetch($datosRecibo2)) {
                    ...
                    $res2 = array("recibo" => $www[0], "fecha" => $www[1], "persona" => $www[2], "ci" => $www[3], "monto" => $resultado33['monto'], "id" => $www[5],"transaccion" => $www[6],"codigotransaccion" => $idtr['codigotransaccion'],"nombre_archivo" => $www[7],"lugar" => $www[8],"nit" => $cl['nit'],"direccion" => $cl['direccion'],"concepto" => $www['concepto'],"estado_documento" => $estado_documento,"registro_desde" => $www['registro_desde']);
                    array_push($lista, $res2);
                }
    
            }else{
                //SOLO HAY INDIVIDUALES
                $registro = $this->dbc->query("SELECT c.idcuentaspof,c.nrecibo,c.fecha,c.monto,c.persona,c.ci,c.transaccion,c.archivo,c.lugar,c.concepto,c.estado,c.registro_desde FROM cuentaspof as c WHERE c.idfactura='$idfactura'");
                while ($qwe = $this->dbc->fetch($registro)) {
                    ...
                    $res = array("id" => $qwe[0], "recibo" => $qwe[1], "fecha" => $qwe[2], "monto" => $qwe[3], "persona" => $qwe[4], "ci" => $qwe[5],"transaccion" => $qwe[6],"codigotransaccion" => $idtr['codigotransaccion'],"nombre_archivo" => $qwe[7],"lugar" => $qwe[8],"nit" => $cl['nit'],"direccion" => $cl['direccion'],"concepto" => $qwe['concepto'],"estado_documento" => $estado_documento,"registro_desde" => $qwe['registro_desde']);
                    array_push($lista, $res);
                }
        }
        }elseif($hayGrupales > 0){
                       ...
                       $resultado3 = $listaGrup->fetch_assoc();
                       $idrecibo = $resultado3['idcuentaspof'];
                       $datosRecibo = $this->dbc->query("SELECT nrecibo,fecha,persona,ci,monto,idcuentaspof,transaccion,archivo,concepto,estado,registro_desde FROM cuentaspof WHERE idcuentaspof='$idrecibo'");
                       ...
                       if($zxc['estado'] == '1'){ //ACTIVO
                        ...
                        $idtr = $trans->fetch_assoc();
                           ...
                           $res = array("recibo" => $zxc[0], "fecha" => $zxc[1], "persona" => $zxc[2], "ci" => $zxc[3], "monto" => $resultado3['monto'], "id" => $zxc[5],"transaccion" => $zxc[6],"codigotransaccion" => $idtr['codigotransaccion'],"nombre_archivo" => $zxc[7],"nit" => $cl['nit'],"direccion" => $cl['direccion'],"concepto" => $zxc['concepto'],"estado_documento" => $estado_documento,"registro_desde" => $zxc['registro_desde']);
                           array_push($lista, $res);
                       }
