...
//------------------------------------------------------------------------------
        $registro = $this->dbc->query("INSERT INTO `factura` (`idfactura`, `fecha`, `nfactura`, `nautorizacion`, `codigocontrol`, `montofactura`, `tasa0`, `export`, `npoliza`, `iceiecdhotros`, `descuentobonificacion`,`tipo_factura`, `clasefactura`, `cobrado`, `pagado`,`idotras_cuentas`, `espesificacion`, `estado`, `tipocompra`, `transacciones_idtransacciones`, `proveedorcliente_idproveedorcliente`, `idorganizacion`, `cuenta`, `sucursal`,`por_concepto_de`,`registro_desde`) 
        VALUES (NULL, '$fecha', '$nfactura', '$nautorizacion', '$codigocontrol', '$monto', '$tasacero', '$export', '$npoliza', '$ice', '$descuento','$tipo_factura', '$clasefactura', '$co', '$pa','0', '$espesificacion', '1', '1', '$idtrans', '$cliente', '$idempresa', '$id_cuenta', '$idsucursal','$por_concepto_de','tributario_x_cobrar');");
        ...
        if($trans == "" && $asiento == ""){
            ...
            VALUES('$nroRecibo','$fecha_completa','1','lugar por defecto','varios clientes','$clientSelect[nombre]','$clientSelect[nit]','$monto','$idfact','0','0','0',NULL,'tributario_cobrado')");
        ...
        if($cuenta == ""){ // SOLO SE ASIGNARA TRANSACCION Y NO LA CUENTA
            ...
            $crearRecibo = $this->dbc->query("INSERT INTO cuentaspof(nrecibo,fecha,estado,lugar,cliente,persona,ci,monto,idfactura,idotras_cuentas,transaccion,cuenta,archivo,registro_desde)
            VALUES('$nroRecibo','$fecha_completa','1','lugar por defecto','varios clientes','$clientSelect[nombre]','$clientSelect[nit]','$monto','$idfact','0','$trans','0',NULL,'tributario_cobrado')");
            ...
            $crearRecibo = $this->dbc->query("INSERT INTO cuentaspof(nrecibo,fecha,estado,lugar,cliente,persona,ci,monto,idfactura,idotras_cuentas,transaccion,cuenta,archivo,registro_desde)
            VALUES('$nroRecibo','$fecha_completa','1','lugar por defecto','varios clientes','$clientSelect[nombre]','$clientSelect[nit]','$monto','$idfact','0','$trans','$cuenta',NULL,'tributario_cobrado')");
...
//------------------------------------------------------------------------------
        $registro = $this->dbc->query("INSERT INTO `factura` (`idfactura`, `fecha`, `nfactura`, `nautorizacion`, `codigocontrol`, `montofactura`, `tasa0`, `export`, `npoliza`, `iceiecdhotros`, `descuentobonificacion`,`tipo_factura`, `clasefactura`, `cobrado`, `pagado`,`idotras_cuentas`, `espesificacion`, `estado`, `tipocompra`, `transacciones_idtransacciones`, `proveedorcliente_idproveedorcliente`, `idorganizacion`, `cuenta`, `sucursal`,`por_concepto_de`,`registro_desde`) 
        VALUES (NULL, '$fecha', '$nfactura', '$nautorizacion', '$codigocontrol', '$monto', '$tasacero', '$export', '$npoliza', '$ice', '$descuento','$tipo_factura', '$clasefactura', '$co', '$pa','0', '$espesificacion', '1', '1', '$idtrans', '$cliente', '$idempresa', '$id_cuenta', '$idsucursal','$por_concepto_de','tributario_cobrado');");
        
        $idfact = $this->dbc->insert_id;

        $crearRecibo = $this->dbc->query("INSERT INTO cuentaspof(nrecibo,fecha,estado,lugar,cliente,persona,ci,monto,idfactura,idotras_cuentas,transaccion,cuenta,archivo,registro_desde)
            VALUES('$nroRecibo','$fecha_completa','1','lugar por defecto','varios clientes','$clientSelect[nombre]','$clientSelect[nit]','$monto','$idfact','0','$idtrans','$id_cuenta',NULL,'tributario_cobrado')");
    ...
    public function registrar_factura_cobros_transaccion($idcajas_bancos,$fecha, $nfactura, $nautorizacion, $codigocontrol, $monto, $tasacero, $export, $npoliza, $ice, $descuento,$tipo_factura, $espesificacion, $cliente, $cobro, $pagar, $trans, $clasefactura, $cuenta, $empresa, $sucursal,$por_concepto_de,$zn)
    {
        ...
        if($cobro == 1){
            ...
            $crearRecibo = $this->dbc->query("INSERT INTO cuentaspof(nrecibo,fecha,estado,lugar,cliente,persona,ci,monto,idfactura,idotras_cuentas,transaccion,cuenta,archivo,registro_desde)
            VALUES('$nroRecibo','$fecha_completa','1','lugar por defecto','varios clientes','$clientSelect[nombre]','$clientSelect[nit]','$monto','$idfact','0','$trans','0',NULL,'transaccion_cobrado')");
    ...
    public function cambiarEstado_anular_eliminar_activar_factura_caja_bancos($idsoli,$estado_opcion,$estado_solicitud,$fecha_proceso,$hora_proceso,$idusuario_admin){
        ...
        $usuario=$this->getidusuario($idusuario_admin);
            ...
            if($estado_opcion == 1){ //ANULAR
                ...
                $res = array("success", "Se Acepto Permiso para Eliminar", "cambiarEstado_anular_eliminar_transaccion");
                    ...
                    if($solicitud['tipo_documento'] == 'factura'){// ES FACTURA
                        ...
                        $reci = $this->dbc->query("SELECT * FROM recibo WHERE idrecibo='$id_documento'");
                        $rec = $reci->fetch_assoc();

                        //ESTAMOS DENEGANDO LOS COMPROBANTES DE LA FACTURA QUE SE QUERIA ANULAR
                        if($rec['cobrado'] != '0'){ // COBROS
                            $update_comprob=$this->dbc->query("UPDATE cuentaspof SET estado = '1' 
                            WHERE idrecibo = '$id_documento'"); 
                        }else{ // PAGOS
                            $update_comprob=$this->dbc->query("UPDATE cuentaspor SET estado = '1' 
                    ...
                    $res = array("success", "Se Denego el permiso para eliminar", "cambiarEstado_anular_eliminar_documento");
                }
...
//                     if($rec['cobrado'] != '0'){ // SI ES COBRADO
...
//                     if($fac['clasefactura'] == '2'){ // SI ES COBRO
...
//                         if($cp['idfactura'] != '0'){ // ES FACTURA
//                             $fac =$this->dbc->query("SELECT * FROM factura WHERE idfactura = '$cp[idfactura]'");
...
//                             $nombre_cl = $cl['nombre'];
//                         }else{ // ES RECIBO
//                             $rec =$this->dbc->query("SELECT * FROM recibo WHERE idrecibo = '$cp[idrecibo]'");
...
public function listar_anular_eliminar_factura($empresa, $todos) {
    ...
    if ($todos == '0') { // solo de la gestion activa
        $sql = $this->dbc->query("
            SELECT * FROM solicitud_anular_eliminar_documento s
            WHERE s.idempresa = '$ide' 
            AND s.idgestion = '$idgestion'
            ORDER BY 
                (s.estado_solicitud = '1') DESC,
    ...
    // Procesar los resultados
    while ($qwe = $sql->fetch_assoc()) {
        $usuario = $this->getusuario($qwe['idusuario']);
        $usuario_admin = $this->getusuario($qwe['idusuario_admin']);
        $nro_doc = null;
        $nombre_cl = null;
        ...
        if ($qwe['tipo_documento'] == 'recibo') {
            ...
            if ($cp = $comprobante->fetch_assoc()) {
                ...
                if ($cp['idfactura'] != '0') {
                    ...
                    $rec = $this->dbc->query("SELECT * FROM recibo WHERE idrecibo = '{$cp['idrecibo']}'")->fetch_assoc();
                    $cliente_prov = $this->dbcm->query("SELECT * FROM cliente WHERE id_cliente = '{$rec['cliente_proveedor']}'");
                    $cl = $cliente_prov->fetch_assoc();
                    $nombre_cl = $cl['nombre'] ?? null;
                }
...
public function registrar_anular_eliminar_activar_factura_tributario_transaccion($id_documento,$registro_desde,$motivo,$estado_opci,$estado_soli,$hora,$fecha,$usuario,$empresa)
    ...
    //     error_reporting(E_ALL);
        ...
        if($fact_rec_tip['cobrado'] != '0'){ // LA FACTURA SERA DE COBRO
            ...
            $comprob = $this->dbc->query("SELECT * FROM cuentaspof WHERE idfactura = '$id_documento'");
            ...
            if($comprob->num_rows > 0){ // EXISTEN COMPROBANTES DENTRO DE LA FACTURA
                ...
                // $editar=$this->dbc->query("UPDATE $nombre_documento_aux SET estado = '2' WHERE $id_documento_nombre = '$id_documento'");  
                    ...
                    while ($zxc = $this->dbc->fetch($comprob)) {
                        $array_lista[] = $zxc['idcuentaspof']; // agrega cada valor al array
                    }
            ...
            $comprob = $this->dbc->query("SELECT * FROM cuentaspor WHERE idfactura = '$id_documento'");
            // $comprobante = $this->dbc->fetch($comprob);
             ...
             if($comprob->num_rows > 0){ // EXISTEN COMPROBANTES DENTRO DE LA FACTURA
                    ...
                    $array_lista = [];
                    while ($zxc = $this->dbc->fetch($comprob)) {
                        $array_lista[] = $zxc['idcuentaspor']; // agrega cada valor al array
                    }
                    $array_string = implode(",", $array_lista);
                }else{ // NO EXISTEN COMPROBANTES DENTRO DE LA FACTURA
        ...
        $registro_factura=$this->dbc->query("INSERT INTO solicitud_anular_eliminar_documento(id_documento, tipo_documento, registro_desde, motivo, estado_opcion, estado_solicitud, hora,fecha, idusuario, idempresa, idgestion)
        VALUES('$id_documento','factura','$registro_desde','$motivo','$estado_opci','$estado_soli','$hora','$fecha','$idusuario','$idempresa','$idgestion')");
        ...
        if($registro_factura===TRUE){
            if($estado_opci == 1){ //estado_opcion= 1 anular
                ...
                $comprob_aux = $this->dbc->query("SELECT * FROM $nombre_comprobante WHERE idfactura = '$id_documento'");
                if($comprob_aux->num_rows > 0){ // EXISTEN COMPROBANTES DENTRO DE LA FACTURA
                    $editar=$this->dbc->query("UPDATE factura SET estado = '2' WHERE idfactura = '$id_documento'");    
                ...
                $comprob_aux = $this->dbc->query("SELECT * FROM $nombre_comprobante WHERE idfactura = '$id_documento'");
                if($comprob_aux->num_rows > 0){ // EXISTEN COMPROBANTES DENTRO DE LA FACTURA
                    $editar=$this->dbc->query("UPDATE factura SET estado = '3' WHERE idfactura = '$id_documento'");    
                    $editar_comprobante=$this->dbc->query("UPDATE $nombre_comprobante SET estado = '3' WHERE $idcomprobante_nombre IN($array_string)");  
                }else{ // NO EXISTEN COMPROBANTES
                    $editar=$this->dbc->query("UPDATE factura SET estado = '3' WHERE idfactura = '$id_documento'");    
             ...
             // estado_factura = 5--> pendiente de activacion 
            //  $editar=$this->dbc->query("UPDATE factura SET estado = '5' WHERE idfactura = '$id_documento'");  
            //  $editar_comprobante=$this->dbc->query("UPDATE $nombre_comprobante SET estado = '5' WHERE $idcomprobante_nombre = '$idcomprobante'");  
            $comprob_aux = $this->dbc->query("SELECT * FROM $nombre_comprobante WHERE idfactura = '$id_documento'");
                if($comprob_aux->num_rows > 0){ // EXISTEN COMPROBANTES DENTRO DE LA FACTURA
                    $editar=$this->dbc->query("UPDATE factura SET estado = '5' WHERE idfactura = '$id_documento'");    
                    $editar_comprobante=$this->dbc->query("UPDATE $nombre_comprobante SET estado = '5' WHERE $idcomprobante_nombre IN($array_string)");  
                }else{ // NO EXISTEN COMPROBANTES
                    $editar=$this->dbc->query("UPDATE factura SET estado = '5' WHERE idfactura = '$id_documento'");    
            ...
            $res = array("success", "Modificacion exitosa","anular_transaccion",$id_documento,'factura',$registro_desde,$motivo,$estado_opci,$estado_soli,$hora,$fecha,$usuario,$empresa);
        }else{
            $res = array("danger", "No se pudo anular");
        }
        ...
        $usuario=$this->getidusuario($idusuario_admin);
            ...
            if($estado_opcion == 1){ //ANULAR
                ...
                if($estado_solicitud == 2){ //ACEPTADO
                    ...
                    //NO SE ANULARA PERO SI CAMBIARA ESTADO DE FACTURA
    ...
    //     error_reporting(E_ALL);
        ...
        if($tipo_documento == 'comprobante_cobro'){ // EL COMPROBANTE SERA DE COBRO
    
            $nombre_comprobante = 'cuentaspof';
            $idcomprobante_nombre = 'idcuentaspof';
            // $tipo_comprobante = 'comprobante_pago';

        }else{ // EL COMPROBANTE SERA DE PAGO
            // $comprob = $this->dbc->query("SELECT * FROM cuentaspor WHERE idfactura = '$id_documento'");
            // $comprobante = $this->dbc->fetch($comprob);
            $nombre_comprobante = 'cuentaspor';
            $idcomprobante_nombre = 'idcuentaspor';
        ...
        if($registro_factura===TRUE){
            if($estado_opci == 1){ //estado_opcion= 1 anular
                // estado_factura = 2--> pendiente de anulacion 
                    ...
                    $editar_comprobante=$this->dbc->query("UPDATE $nombre_comprobante SET estado = '2' WHERE $idcomprobante_nombre = '$id_documento'");  

            }elseif($estado_opci == 2){ //estado_opcion= 2 eliminar
                // estado_factura = 3--> pendiente de eliminacion 
               
                    $editar_comprobante=$this->dbc->query("UPDATE $nombre_comprobante SET estado = '3' WHERE $idcomprobante_nombre = '$id_documento'");  

            }else{//estado_opcion= 3 activar
             // estado_factura = 5--> pendiente de activacion 

                    $editar_comprobante=$this->dbc->query("UPDATE $nombre_comprobante SET estado = '5' WHERE $idcomprobante_nombre = '$id_documento'");  
                
            }
            
            $res = array("success", "Solicitud exitosa","anular_transaccion",$id_documento,'comprobante',$registro_desde,$motivo,$estado_opci,$estado_soli,$hora,$fecha,$usuario,$empresa);
        }else{
            $res = array("danger", "No se pudo realizar la operacion");
        }
            ...
            if($estado_opcion == 1){ //ANULAR
                ...
                if($estado_solicitud == 2){ //ACEPTADO
                        ...
                        if($tipo_documento == 'comprobante_cobro'){
                            ...
                            $eliminar_dt_compr=$this->dbc->query("DELETE FROM detalle_caja_bancos_cobrar
                            WHERE idcuentaspof = '$id_documento'"); 
                        }else{
                            $eliminar_comprob=$this->dbc->query("DELETE FROM cuentaspor
                            WHERE idcuentaspor = '$id_documento'");  
