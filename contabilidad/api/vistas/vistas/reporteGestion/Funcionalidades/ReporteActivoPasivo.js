...
/**
 * Función: Prepara y maneja el reporte de activo y pasivo.
 * Descripción: Esta función obtiene los datos necesarios para el reporte de activo y pasivo.
 ...
 * Autor: Joel Choque
 */
export async function manejarReporteActivoPasivo(datos) {
    const {
        urlSolicitud,
        datosFormulario,
        vistaReporte,
    ...
    const opcionesReporte = opcionesDescargaDeReporte({
        ...
        // cuerpoReporte,
        // nombreArchivo: `reporte_activo_y_pasivo','reporte_activo_y_pasivo_hasta`,
        callbackPDF: async (numeroPieDePagina) => {
            const docDefinition = await reporteActivoPasivoPdfMake({ datosReporte, fechaReporte });
            await defDocumentoPdfMake({ contenido: docDefinition, numeroPieDePagina });
        },
        callbackExcel: (conEncabezado) => {
            exportarExcelCuentaBalance({
                datos: datosReporte,
    ...
    const reporte = await reporteActivoPasivo({ datosReporte, fechaReporte });
...
// Genera el contenido HTML del reporte de activo y pasivo para vista en pantalla
async function reporteActivoPasivo({ datosReporte, fechaReporte }) {
    ...
    const subtitulo = `
        <div><h4>Detalle de Cuentas de Balance</h4></div>
...
// Genera el contenido para PDFMake del reporte de activo y pasivo para descarga en PDF
async function reporteActivoPasivoPdfMake({ datosReporte, fechaReporte }) {
    if (datosReporte.error) {
        return;
    }
    ...
    content.push(
        { text: "Detalle de Cuentas de Balance", style: "textoTitulo" },
        {
            stack: [
...
// Función para exportar los datos del reporte de activo y pasivo a Excel
function exportarExcelCuentaBalance({datos, encabezado = {conEncabezado: false, fechaReporte: "", divisa: ""}} = {}) {
    ...
    if (encabezado.conEncabezado) {
        ...
        rowLM[0] = { v: "Detalle de Cuentas de Balance", s: styleAccount };
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Cuentas de Balance");
    XLSX.writeFile(wb, `cuentas_de_balance.xlsx`, { compression: true });
}