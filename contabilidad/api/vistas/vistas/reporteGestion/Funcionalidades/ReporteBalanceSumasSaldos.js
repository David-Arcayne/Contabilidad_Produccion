...
/**
 * Función: Prepara y maneja el reporte de balance de sumas y saldos.
 * Descripción: Esta función obtiene los datos necesarios para el reporte de balance de sumas y saldos.
 ...
 * Autor: Joel Choque
 */
export async function manejarReporteBalanceSumasSaldos(datos) {
    const {
        urlSolicitud,
        datosFormulario,
        vistaReporte,
    ...
    const opcionesReporte = opcionesDescargaDeReporte({
        ...
        // cuerpoReporte,
        // nombreArchivo: `balance_sumas_y_saldos','balance_sumas_y_saldos_hasta`,
        callbackPDF: async (numeroPieDePagina) => {
            const docDefinition = await reporteBalanceSumasSaldosPdfMake({ datosReporte, fechaReporte });
            await defDocumentoPdfMake({ contenido: docDefinition, numeroPieDePagina });
        },
        callbackExcel: (conEncabezado) => {
            exportarExcelBalanceSumasSaldos({
                datos: datosReporte,
    ...
    const reporte = await reporteBalanceSumasSaldos({ datosReporte, fechaReporte });
...
// Genera el contenido HTML del reporte de Balance de Sumas y Saldos para vista en pantalla
async function reporteBalanceSumasSaldos({ datosReporte, fechaReporte }) {
    ...
    const subtitulo = `
        <div><h4>Balance de Comprobación de Sumas y Saldos</h4></div>
...
// Genera el contenido para PDFMake del reporte de balance de Sumas y Saldos para descarga en PDF
async function reporteBalanceSumasSaldosPdfMake({ datosReporte, fechaReporte }) {
    if (datosReporte.error) {
        return;
    }
    ...
    content.push(
        { text: "Balance de Comprobación de Sumas y Saldos", style: "textoTitulo" },
        {
            stack: [
...
// Función para exportar los datos del reporte de Balance de Sumas y Saldos a Excel
function exportarExcelBalanceSumasSaldos({datos, encabezado = {conEncabezado: false, fechaReporte: "", divisa: ""}} = {}) {
    ...
    if (encabezado.conEncabezado) {
        ...
        rowLM[0] = { v: "Balance de Comprobación de Sumas y Saldos", s: styleAccount };
    ...
    XLSX.writeFile(wb, `balance_sumas_y_saldos.xlsx`, { compression: true });
}