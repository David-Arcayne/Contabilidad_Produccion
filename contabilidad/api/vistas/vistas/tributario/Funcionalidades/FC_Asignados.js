...
import { pdfMakeFacturasComercialAsignados } from "./ReportesTributario.js";

/**
 * Función: Crea el contenido principal de la vista
 * Descripción: Esta función genera el contenido principal de la vista para la gestión de facturas comerciales asignadas.
 *              Permite visualizar una tabla con las facturas asignadas, junto con opciones de búsqueda y filtrado.
 * Fecha: 18 de febrero de 2026
 * Autor: Joel Choque
 */
export function FC_Asignados({permisos, vistaFacturasComercial, vistaFacturasAsignadas}) {

    const EMPRESA_ID = getEmpresaId();
    const URL = CT_URLAPI;
    const URL_LT = `${URL}listar_factura_comercial_con_transaccion/${EMPRESA_ID}`;
    ...
    vistaFacturasAsignadas.append(vistaPrincipal);

    // Creación de elementos para la vista Principal
    const regresar = async () => {
        cambiarVista(vistaFacturasAsignadas, vistaFacturasComercial);
        vistaFacturasAsignadas.innerHTML = "";
    };
    const encabezadoVista = seccionEncabezado({ titulo: "Facturas Asignadas" }, { callback: regresar });
    ...
    const divFiltros = crearFiltrosTabla({
        obtenerContenidoReporte: pdfMakeFacturasComercialAsignados(controladorTabla.getDatosOriginales),
        controladorTabla,
