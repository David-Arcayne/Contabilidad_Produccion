...
import { pdfMakeFacturasComercialProformas } from "./ReportesTributario.js";

/**
 * Función: Crea el contenido principal de la vista
 * Descripción: Esta función genera el contenido principal de la vista para la gestión de proformas.
 *              Permite visualizar una tabla con las proformas, junto con opciones de búsqueda y filtrado.
 * Fecha: 18 de febrero de 2026
 * Autor: Joel Choque
 */
export function FC_Proformas({ permisos, vistaFacturasComercial, vistaFacturasProformas }) {

    const EMPRESA_ID = getEmpresaId();
    const URL = CT_URLAPI;
    const URL_LT = `${URL}listar_factura_comercial_comprobante/${EMPRESA_ID}`;
    ...
    vistaFacturasProformas.append(vistaPrincipal);

    // Creación de elementos para la vista Principal
    const regresar = async () => {
        cambiarVista(vistaFacturasProformas, vistaFacturasComercial);
        vistaFacturasProformas.innerHTML = "";
    };
    const encabezadoVista = seccionEncabezado({ titulo: "Proformas" }, { callback: regresar });
    ...
    const divFiltros = crearFiltrosTabla({
        obtenerContenidoReporte: pdfMakeFacturasComercialProformas(controladorTabla.getDatosOriginales),
        controladorTabla,
