...
/**
 * Función: Crea la vista de facturas de cobro sin cuenta
 * Descripción: Esta función genera la vista para gestionar las facturas de cobro que no han sido asignadas a una transacción y a su cuenta contable.
 *              Permite seleccionar múltiples facturas y asignarlas a una transacción y cuenta contable mediante un formulario modal.
 * Fecha: 09 de febrero de 2026
 * Autor: Joel Choque
 */
export function FacturasCobroSinCuenta(elementosFxC) {
    const {
        codigo,
        permisos,
    ...
    const URL = CT_URLAPI;
    const URL_LT = `${URL}listar_factura_cobro_sin_cuentas/${EMPRESA_ID}`;
    ...
    const regresar = async () => {
        ...
        cambiarVista(vistaFacturasSinCuenta, vistaFacturacionCobro);
    ...
    function cargarContenidoTabla(listaRegistros) {
        ...
        for (const registro of listaRegistros) {
            ...
            const estado = registro.cobrado === "1" ? "Por cobrar" : ( registro.cobrado === "2" ? "Cobrado" : "-");
...
function columnaCheckbox(datosVista) {
    ...
    input.addEventListener("change", (e) => {
        ...
        botonAsientoOTransaccion.addEventListener("click", async (e) => {
            ...
            const { form, modal, cerrarModal, botonAsignar } = formularioModalAsientoOTransaccion("fcsincuenta", "transaccion", "Asignar a Transacción y Cuenta");
