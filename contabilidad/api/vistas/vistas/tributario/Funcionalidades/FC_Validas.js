...
import { pdfMakeFacturasComercialValidas } from "./ReportesTributario.js";

/**
 * Función: Crea el contenido principal de la vista
 * Descripción: Esta función genera el contenido principal de la vista para la gestión de facturas comerciales válidas.
 *              Permite visualizar una tabla con las facturas válidas, junto con opciones de búsqueda y filtrado.
 * Fecha: 18 de febrero de 2026
 * Autor: Joel Choque
 */
export function FC_Validas({ permisos, vistaFacturasComercial, vistaFacturasValidas }) {
    ...
    vistaFacturasValidas.append(vistaPrincipal);

    // Creación de elementos para la vista Principal
    const regresar = async () => {
        cambiarVista(vistaFacturasValidas, vistaFacturasComercial);
        vistaFacturasValidas.innerHTML = "";
    };
    const encabezadoVista = seccionEncabezado({ titulo: "Facturas Válidas" }, { callback: regresar });
    ...
    const divFiltros = crearFiltrosTabla({
        obtenerContenidoReporte: pdfMakeFacturasComercialValidas(controladorTabla.getDatosOriginales),
        controladorTabla,
