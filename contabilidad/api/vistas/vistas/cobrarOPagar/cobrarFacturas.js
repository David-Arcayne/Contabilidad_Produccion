...
import { formularioTribFacturaCobro } from "../tributario/Formularios.js";
import { pdfMakeDetalleCobrados } from "../tributario/Funcionalidades/ReportesTributario.js";
import { formularioCobrarFacturaMultiple } from "./Formularios.js";
import { CobrarFactura } from "./Funcionalidades/CobrarFactura.js";
import { pdfMakeCobrosDeFacturas } from "./Funcionalidades/ReportesCobroPago.js";
 ...
 * @param {HTMLElement} vistaFacturasPorCobrar - Contenedor principal donde se renderiza la vista de facturación de cobros.
 * @param {DatosMenuBotones} datosVistaPrincipal - Información de la vista: código, permisos, título.
 */
export function CobrarFacturas(permisos, vistaFacturasPorCobrar, datosVistaPrincipal) {
    const PUEDE_ESCRIBIR = permisos.escritura === "1";
    const PUEDE_EDITAR = permisos.editar === "1";
    const EMPRESA_ID = getEmpresaId();
    const SUCURSAL_ID = getSucursalId();
    const USUARIO_ID = getUsuarioId();
    const PREFIJO_ID = "creditocobrarfactura";
    const URL = CT_URLAPI;
    ...
    vistaFacturasPorCobrar.append(vistaPrincipal, vistaCobrar);
        ...
        const botonRegistrar = botonModalRegistro(
            {
                ...
                URL_FORM: URL,
                URL_LISTAR: URL_LT,
                camposDeFormulario: formularioTribFacturaCobro(PREFIJO_ID, false),
                cargarContenidoTabla,
                configuracionModal: {
                    tituloModal: "Registro de Nueva Factura de Cobro",
                },
                camposAdicionales: {
    ...
    const divFiltros = crearFiltrosTabla({
        obtenerContenidoReporte: pdfMakeCobrosDeFacturas(controladorTabla.getDatosOriginales),
        controladorTabla,
    ...
    controladorTabla.suscribir("tabla", (lista) => {
        ...
        const objetoRegistros = {}; // Objeto para almacenar los registros seleccionados para cobro múltiple
        let contador = 0;
        let totalMonto = 0;
        let totalCobrado = 0;
        let totalSaldo = 0;
        for (const item of lista) {
            ...
            totalCobrado += Math.round(parseFloat(registro.pagado || 0) * 100);
            ...
            const botonDetalle = botonDetalleCobros(
                () => {
                    ...
                    CobrarFactura({
                        permisos,
                        vistaCobrosDeFacturas: vistaPrincipal,
                        vistaCobrar,
                        urlCobrosDeFacturas: URL_LT,
                        registroFactura: registro,
                        cargarTablaCobrosDeFacturas: cargarContenidoTabla,
                    });
                }
            );
            ...
            const botonReporte = crearBotonIconoPdfMake({
                contenedorModal: vistaPrincipal,
                obtenerContenido: pdfMakeDetalleCobrados(`${URL}listapagos/${registro.id}`, registro, registro.saldo),
        ...
        [
            {
                popover: {
                    ...
                    description: "Sección para gestionar el cobro de facturas pendientes (crédito).",
                },
            },
            {
            ...
            },
            {
                ...
                popover: {
                    ...
                    description: "Muestra todos los registros de facturas de cobros pendientes en el sistema.",
                },
            },
            {
                mainElement: tabla,
                ...
                popover: {
                    ...
                    description: "Abre una vista donde se puede registrar el cobro de la factura seleccionada. Así mismo se genera automáticamente el Comprobante de Ingreso",
                },
            },
            {
                mainElement: tabla,
...
function botonDetalleCobros(callback) {
    ...
    const boton = crearElemento("button", { class: "btn btn-primary btn-sm", type: "button" }, [icono, "Cobrar"]);
...
/**
 * Función: Crea una columna con checkbox para seleccionar facturas en la tabla de cobros.
 * Descripción: Esta función genera una columna con un checkbox para cada fila de la tabla de cobros.
 *              Permite seleccionar múltiples facturas para realizar un cobro múltiple, también poder agregarlos a una Caja o Banco.
 *              Al seleccionar una factura, se valida que solo se puedan seleccionar facturas del mismo cliente y se muestra un botón para realizar el cobro múltiple.
...
function columnaCheckbox({
    ...
    const PREFIJO_ID = "tccobrofacturamultiple";
    ...
    input.addEventListener("change", (e) => {
        ...
        // Validar que solo se puedan seleccionar facturas del mismo cliente
        const cliente = Object.values(objetoRegistros)?.[0]?.idproveedor || null;
        if (!(cliente === null || cliente === registro.idproveedor)) {
            const modal = modalDeInformacion("Solo puede seleccionar facturas de un mismo cliente");
        ...
        const botonCobrarMultiple = crearElemento("button", {class: "btn btn-info btn-sm"}, [icono, "Realizar Cobro"]);
        const divOpciones = crearElemento("div", {class: "d-flex justify-content-start mb-2", id: "opcion-agrupar"}, [botonCobrarMultiple]);
        vistaPrincipal.insertBefore(divOpciones, divTabla);
        // Evento para abrir el modal de cobro múltiple al hacer click en el botón
        botonCobrarMultiple.addEventListener("click", async (e) => {
            ...
            const [modal, cuerpoModal, cerrarModal] = modalRemovible({
                tituloModal: "Cobro Múltiple de Facturas",
            ...
            if (datosGestion?.nombre) {
                ...
                const h1 = crearElemento("h1", { class: "h6 text-center text-danger-emphasis" }, ["Es necesario tener una gestión activa para realizar cobros."]);
                cuerpoModal.appendChild(h1);
                return;
            }

            // Crear el formulario para el cobro múltiple y preparar los campos.
            const formulario = crearFormulario(formularioCobrarFacturaMultiple(), {
                opcionesParaBotones: {
            ...
            formulario.addEventListener("submit", async (e) => {
                ...
                const datosExtra = {
                    ...
                    ver: "registrocobrarfacturaGrupal",
                    zona_horaria: Intl.DateTimeFormat().resolvedOptions().timeZone,
                    usuario:  usuario_id,
                }
...
/**
 * Función: Valida los campos del formulario de cobro múltiple antes de enviar la solicitud al servidor.
 * Descripción: Esta función valida que la fecha esté dentro del rango de la gestión activa, valida que el monto de las cajas y bancos no sea mayor al monto total a cobrar, entre otras validaciones.
...
async function validarCamposFormulario(vista, gestion, form, arrayDeCajaBancos, monto, PREFIJO_ID) {
    ...
    // Validar que el monto de las cajas y bancos no sea mayor al monto total a cobrar
    const usuario_id = getUsuarioId();
    const empresa_id = getEmpresaId();
    ...
    if (datosCB && datosCB.length > 0 && arrayDeCajaBancos) {
        if (arrayDeCajaBancos.length > 0) {
            ...
            if (total !== monto) {
                const modal = modalDeInformacion("El monto de Cajas y Bancos debe ser igual al monto a cobrar");
                vista.appendChild(modal);
                return false;
            }
        } else {
...
/**
 * Función: Crea un modal para asignar montos a las cajas y bancos al realizar un cobro múltiple de facturas.
 * Descripción: Genera un modal donde se pueden asignar montos a las cajas y bancos disponibles para el cobro múltiple de facturas.
 *              Permite seleccionar la factura a la que se le asignará el monto, así como la caja o banco correspondiente.
 *              Al agregar una asignación, se muestra en una tabla dentro del modal y se valida que el monto total asignado no sea mayor al monto total a cobrar.
 * Fecha: 04 de marzo de 2026
 * Autor: Joel Choque
 */
        ...
        <form class="row g-2 mt-0">
            <div>
                <div class="col-12 col-md-6">
                    ...
                    <select class="form-select" id="m_cb_facturas" name="factura" required data-iclear="false">
                    </select>
                </div>
            </div>
            <div class="col-12 col-md-6">
