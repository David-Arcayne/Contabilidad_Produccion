 ...
 * Descripción: Esta función genera el contenido principal del menú para la gestión de contratos de cobro cobrados.
 * Fecha: 21 de febrero de 2026
 * Autor: Joel Choque
 */
export async function ListaCobrados(datosVista) {
    const {
        permisos,
    ...
    vistaContratosCobrados.append(vistaPrincipal);
    ...
    const divNombre = crearElemento("div", undefined, [crearElemento("b", undefined, ["Cliente:"]), ` ${registro.nombrep}`]);
    const spanMonto = crearElemento("span", { class: "fw-normal", id: "cdc-cobros-span-saldo" }, [formatoDecimal(registro.precio)])
    ...
    const regresar = async () => {
        ...
        cambiarVista(vistaContratosCobrados, vistaDocumentosDeCobro)
        vistaContratosCobrados.innerHTML = "";
    };
    const encabezadoVista = seccionEncabezado(
        { titulo: "Detalle de Cobros", textoInformacion: moneda, elementoAdicional: divColumnas  },
        { callback: regresar }
    );
