...
import { pdfMakeContratosDePago } from "./ReportesContrataciones.js";

/**
 * Función: Crea el contenido principal del menú
 * Descripción: Esta función genera el contenido principal del menú para la gestión de contratos de pago vencidos.
 * Fecha: 21 de febrero de 2026
 * Autor: Joel Choque
 */
export function ContratosDePagoVencidos(datosVistaContratos) {
    const {
        codigo,
        permisos,
    ...
    const URL = CT_URLAPI;
    const URL_LT = `${URL}listar_otras_cuentas_pagar_vencidas/${EMPRESA_ID}`;
    ...
    const regresar = async () => {
        ...
        cambiarVista(vistaContratosVencidos, vistaDocumentosDePago)
        vistaContratosVencidos.innerHTML = "";
    };
    const encabezadoVista = seccionEncabezado({titulo: "Contratos de Pago Vencidos"}, { callback: regresar });
    ...
    const divFiltros = crearFiltrosTabla({
        obtenerContenidoReporte: pdfMakeContratosDePago(controladorTabla.getDatosOriginales, "Contratos de Pago Vencidos"),
                    ...
                    const botonEditar= botonModalEditar(
                        {
                            ...
                            URL_LISTAR: URL_LT,
                            configuracionModal: {
                                tituloModal: "Edición de Contrato de Pago Vencido",
                            },
                            camposAdicionales: {
