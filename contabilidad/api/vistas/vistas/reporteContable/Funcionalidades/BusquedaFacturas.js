...
/**
 * Función: Prepara y maneja el reporte de búsqueda de facturas.
 * Descripción: Esta función obtiene los datos necesarios para el reporte de búsqueda de facturas.
 ...
 * Autor: Joel Choque
 */
export async function manejarReporteBusquedaFactura(datos) {
    const {
        urlSolicitud,
        datosFormulario,
        vistaReporte,
    ...
    const opcionesReporte = opcionesDescargaDeReporte({
        ...
        callbackPDF: async (numeroPieDePagina) => {
            const docDefinition = await reporteBusquedaFacturaPdfMake({ datosReporte, filtrosAplicados: filtrosAplicados.filtroPdfMake });
            await defDocumentoPdfMake({ contenido: docDefinition, numeroPieDePagina });
        },
        callbackExcel: (conEncabezado) => {
            exportarExcelFiltroFacturas({
                datos: datosReporte,
    ...
    const reporte = await reporteBusquedaFactura({ datosReporte, filtrosAplicados: filtrosAplicados.filtroHtml });
...
// Genera el contenido HTML del reporte de búsqueda de facturas
async function reporteBusquedaFactura({ datosReporte, filtrosAplicados }) {

    let htmlFirmas = "";

    const subtitulo = `
        <div><h4>Facturas</h4></div>
    ...
    for (const txn of datosReporte) {
        saldo += Math.round(parseFloat(txn.montofactura || 0) * FACTOR);
        contador++;
        htmlTable += `
            <tr>
                <td>${contador}</td>
                ...
                <td>${txn.nit || "-"}</td>
                <td class="afr-te">${formatoDecimal(txn.montofactura)}</td>
                <td>${txn.estado_ || "-"}</td>
...
// Genera el contenido para PDFMake del reporte de facturas para descarga en PDF
async function reporteBusquedaFacturaPdfMake({ datosReporte, filtrosAplicados }) {
    ...
    for (const txn of datosReporte) {
        contador++;
        const precioTF = Math.round(parseFloat(txn.montofactura || 0) * FACTOR);
        saldo += precioTF;
    ...
    content.push(
        { text: "Facturas", style: "textoTitulo" },
        {
            stack: [
...
// Función para exportar los datos del reporte de búsqueda de facturas a Excel
export function exportarExcelFiltroFacturas({datos, encabezado = {conEncabezado: false, divisa: ""}} = {}) {
    ...
    if (encabezado.conEncabezado) {
        ...
        rowLM[0] = { v: "Facturas", s: styleAccount };
    ...
    for (const value of datos) {
        contador ++;
        const monto = Math.round(parseFloat(value.montofactura) * FACTOR);
    ...
    XLSX.writeFile(wb, `facturas.xlsx`, { compression: true });
}